...
- 前端闸门（managed Node 22.22.2；⚠️ 勿用 `node_modules/.bin/*`——是 bash 包装会被当 JS 执行）：
  - `tsc --noEmit` → `typescript/bin/tsc`；`eslint src --ext .ts,.tsx,.js,.jsx` → `eslint/bin/eslint.js`；`prettier --check "src/**/*.{ts,tsx}"` → 入口 **bin-prettier.js**；`vitest run` → `vitest/vitest.mjs`（**全量勿加 `--pool=forks`**：Windows+中文路径 forks 慢且吃内存）
...
## 前端类型 / 结构规范
- **`tsconfig.json` 已全量严格（2026-09-11 逐格收口）：`"strict": true` + `"noImplicitOverride": true`**（原逐项 `noImplicitAny`/`strictNullChecks` 已删，被 strict 涵盖）。新代码必须类型干净（`tsc --noEmit` 0 错误）；**禁新增 `any`**；可空/可选须显式处理（仅「保持原运行时值不变」处用 `as`，禁靠 `!` 批量绕过）。
  - ⚠️ **容器签名铁律**：接收「任意组件」的 importFn 参数写 `React.ComponentType`（默认泛型 `{}`），**禁写 `ComponentType<unknown>`** —— 泛型处于 props **逆变位**，`unknown` 不能赋给页面 `{}` props，会让全部 `React.FC` 页面报 TS2322。**判据：同批调用点里只有「显式标注 `React.FC` 的组件」报错 ⇒ 根因在容器、不在组件。**
  - ❌ **明确不推**（已量化登记，会改变运行时语义）：`noUncheckedIndexedAccess`(133) / `exactOptionalPropertyTypes`(157) / `noPropertyAccessFromIndexSignature`(187)；`noUnusedLocals`(13)/`noUnusedParameters`(7) 与 eslint `no-unused-vars` 重叠。
  - **每格先 `tsc --noEmit --<flag>` 单独量化再决定**：#112 实测代价 100% 集中在 `strictFunctionTypes`（18）、其余 5 格全 0 → 直接写 `strict:true`，不逐 flag 堆叠。
...
- **导入一律走 barrel**（细节见 skill `frontend-import-barrel-unification`）：hooks → `'.../hooks'`；components → `'.../components'`（**双层**：根聚合 9 个子 barrel ui/data-display/feedback/form/layout/image/lazy/special/workbench）。新增子目录**必须建 `index.ts` 并在根 barrel 同步聚合**；依赖重的子目录（charts/recharts）保留独立入口；子 barrel 转发**必须按真实导出形式**，不可统一 `default as X`。
- ⚠️ **批量改 import 脚本铁律**：① 匹配条件须含**已有 barrel import**；② 替换段**只吃匹配语句本身**，禁用 `raw[:a]+raw[b:]` 跨越删除（会吃掉中间 import → TS2304）；③ 先 dry-run；④ 动态 `import()` 须单独扫。
- ⚠️ **误删恢复 SOP**：**不以 `git checkout` 救**（丢其它未提交改动）→ **以 tsc 报错名（TS2304/TS2552）为权威**（HEAD 导入集差集会大量误报）→ 全仓导出索引回退 + HEAD import 映射 → 修完四项横向体检。
...
- ⚠️ **EOL 铁律**（详见 skill `black-batch-eol-safe`）：backend 大量 `.py` 为 CRLF，**禁 Edit 直改**（会整文件转 LF → 噪音 diff）；须 python 二进制读改写。**判据 = 字节计数**（`b.count(b"\r\n")`），勿信 `grep -c $'\r'`（Git Bash 假阳性）；**逐文件判**。
- ⚠️ **ruff 用默认 ll=88**（仅 black 是 100）→ 折行类自动修受 ll 门控（折叠后 >ll 即拒修，需 `--line-length 1000` 再折回）；**black 仅对 `--check` 已通过的文件跑**（否则顺带重排无关行）。
- ⚠️ **`%` → f-string 批量转换须双守卫**：① AST 全扫「非 f 字符串含 `{identifier` 占位」（曾漏 `f` 前缀致输出字面量）；② 内容级测试断言（勿只验结构）。

## 业务模块
- **NLP**（✅ 2026-08-29）：链路 `api/nlp/nlp_routes.py::_get_parser()` → `services/nlp_enhanced_service.get_nlp_parser()`（`nlp_service.py` 仅预热）；torch 懒加载（首次 `ml_predict`）。G5 OpenAPI 469 零漂移。
- **班主任工作台**（✅ 全闭环，2026-09-11 实测复核）：`useWorkbenchClass`（store + `useSyncExternalStore`，12 子页共享当前班级、sessionStorage 持久）；评语 `TeacherComment` → `/api/teacher-comments`（`comment.view/edit`）。4 条硬要求：字段命名统一 / 权限体系 / **字段·权限·业务逻辑变更须经用户审核** / 优化交互展示。聚合首页已落地（`/workbench` → `pages/WorkbenchOverview`；菜单「工作台总览」；未匹配路径默认重定向；`ENTRIES` 13 + `GLOBAL_ENTRIES` 4）。**无待办开发项。**

## ⚠️ 审计文档引用铁律（2026-09-11 教训）
- **引用 `docs/` 历史审计文档前必须先实测复核**——生成日期 ≠ 当前状态（曾因采信 08-29 文档 + `grep class.view` 的**行号**而错报 P0/P1 未修）。
- **grep 权限词根必须带 `-A3` 并核对所属路由**，前端 `requiredPermission` ↔ 后端 `@requires_permission("X")` 逐路由比对；「某文件出现过 X」不可作判据。
- **判断「某属性无消费者」须按类型归属逐一核对**（`grep "\.rule_id"` 命中 8 处却分属 `Suggestion`/`MatchedRule`/误加者）——**判据 = 该属性所属接口上的全部读取点**。
...
## A 轨列表钩子迁移 SOP（2026-09-05 固化，细节见当日日志）
- 选型：服务端**分页**列表 → `useListFetch`；**全量下拉/选项** → `useListData`（`data` 恒数组）。底层 `useOptimizedFetch` 的 `enabled=false` 用于模态/切 tab 按需加载（两 hook 均透传）。
- 六步：① 取证 ② 删手工态 ③ params 声明式注入 ④ 乐观更新→`mutate` ⑤ handler 包 `useCallback` 返 `Promise<void>` ⑥ 渲染全量映射。
- 判定：✅ 纯 reload / 乐观增删 / 伪命令式；❌ **真命令式**（如 `fetchClasses(page,keyword,skipCache,perPage)` 绕后端缓存，需先扩 hook）。
- 踩坑：`fetchJson` 失败返 null 不抛 → fetcher 内转抛；列表与表单共享 isLoading 须拆两层；迁移后必跑**全量** vitest。
- 已迁 13 页（清单见 `memory/2026-09-05.md`）。
