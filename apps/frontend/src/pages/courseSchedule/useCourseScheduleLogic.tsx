...
import { useCourseImportExport } from './useCourseImportExport';

/**
 * 课程表页逻辑 hook（组合根：全局状态 / 数据加载 / 冲突检测 / 提交；导入导出域已下沉）。
 */
export function useCourseScheduleLogic(): CourseScheduleViewProps {
  ...
  // —— 导入 / 导出域（状态 + 专用弹窗 + 处理器已下沉）——
  const importExport = useCourseImportExport({
    showToast,
    selectedClass,
    fetchData,
