...
export const defaultCultureForm: CultureFormData = {
  id: null,
  category: '标语',
  title: '',
  content: '',
...
export const cultureCategories = ['全部', '标语', '规则', '荣誉', '其他'];
...
interface CultureBoardViewProps {
  activeCategory: string;
  groupedRecords: Record<string, CultureRecord[]>;
  isLoading: boolean;
  culturePage: number;
  cultureTotal: number;
  setCulturePage: (p: number) => void;
  ...
  onOpenCreate: () => void;
  onOpenEdit: (r: CultureRecord) => void;
  onCloseModal: () => void;
  onSubmit: () => void;
  onDelete: (id: number) => void;
  onMoveOrder: (id: number, direction: 'up' | 'down') => void;
  onChange: (field: keyof CultureFormData, value: string | number | boolean) => void;
}

const CultureBoardView: React.FC<CultureBoardViewProps> = ({
  activeCategory,
  groupedRecords,
  isLoading,
  culturePage,
    ...
    >
      <div className='flex items-start justify-between mb-2'>
        ...
        <div className='flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity'>
          <button
            onClick={() => onMoveOrder(record.id, 'up')}
          ...
          <button
            onClick={() => onMoveOrder(record.id, 'down')}
            className='p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-all'
            title='下移'
          >
  ...
  return (
    <div className='flex flex-col h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800'>
      ...
      <div className='flex-1 px-6 pb-6 overflow-y-auto'>
        {isLoading ? (
        ...
        ) : Object.values(groupedRecords).every((list) => list.length === 0) ? (
          <EmptyState
            icon='folder'
            title='暂无文化记录'
          ...
          <div className='space-y-6'>
            {cultureCategories.slice(1).map((cat) => {
...
export default CultureBoardView;
