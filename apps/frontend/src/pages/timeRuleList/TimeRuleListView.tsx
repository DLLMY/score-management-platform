import React, { useMemo, useCallback, type FormEvent, type ChangeEvent } from 'react';
import {
  Plus,
  Edit2,
  Trash2,
...
import { Button, PermissionButton, DataTable, type ColumnType } from '../../components';
...
export interface TimeRuleFormData {
  name: string;
  description: string;
  day_of_week: number;
  start_hour: number;
...
export interface TimeRuleFormErrors {
  name?: string;
  description?: string;
  start_hour?: string;
  start_minute?: string;
...
interface TimeRuleListViewProps {
  loading: boolean;
  loadError: boolean;
  filteredRules: TimeRule[];
  searchTerm: string;
  ...
  showModal: boolean;
  closeModal: () => void;
  editingRule: TimeRule | null;
  formData: TimeRuleFormData;
  setFormData: (
    data: Partial<TimeRuleFormData> | ((prev: TimeRuleFormData) => Partial<TimeRuleFormData>)
  ) => void;
  formErrors: TimeRuleFormErrors;
  setFormErrors: (errors: TimeRuleFormErrors) => void;
  submitting: boolean;
...
const TimeRuleListView: React.FC<TimeRuleListViewProps> = ({
  loading,
  loadError,
  filteredRules,
  searchTerm,
      ...
      },
      {
        ...
        render: (_, rule) => (
          <span className='text-gray-600'>
            {formatHourMinute(rule.start_hour, rule.start_minute)} -{' '}
            {formatHourMinute(rule.end_hour, rule.end_minute)}
          </span>
        ),
      },
      {
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex flex-col lg:flex-row lg:items-center justify-between gap-4'>
        ...
        <PermissionButton permission='timetable.rule.manage' onClick={onAdd}>
...
export default TimeRuleListView;
