...
import type { StudentPortalViewProps, TabKey } from './types';
import { StudentGrowthTab } from './components';
  ...
  return (
    <div className='min-h-screen bg-gray-50 dark:bg-slate-900'>
      ...
      <main className='p-4 max-w-3xl mx-auto space-y-4'>
          ...
          <>
            ...
            <div className='bg-white dark:bg-slate-800 rounded-2xl p-4 shadow'>
              <div className='flex items-center justify-between mb-3'>
                ...
                >
                  <RefreshCw
                    className={`w-3.5 h-3.5 ${scoreRecords.loading ? 'animate-spin' : ''}`}
                  />{' '}
                  刷新
                </button>
              </div>
          ...
          <div className='bg-white dark:bg-slate-800 rounded-2xl p-4 shadow'>
            <div className='flex items-center justify-between mb-3'>
              <div className='flex items-center gap-2 font-semibold text-gray-800 dark:text-white'>
                ...
                {notifList.total > 0 && (
                  <span className='text-xs text-gray-400'>({notifList.total})</span>
                )}
              </div>
              <button
                ...
                className='text-sm text-primary-500 flex items-center gap-1 disabled:opacity-50'
              >
                <RefreshCw className={`w-3.5 h-3.5 ${notifList.loading ? 'animate-spin' : ''}`} />{' '}
                刷新
              </button>
            </div>
        ...
        {tab === 'growth' && <StudentGrowthTab insights={insights} growthLoading={growthLoading} />}
      </main>
    </div>
  );
}
