...
import type { ClassCompareViewProps } from './types';

/**
 * 班级对比分析视图层：班级选择、周期切换、指标图表与详细面板。
 * 状态与数据加载逻辑见 ../ClassCompare。
 */
export default function ClassCompareView({
  loadError,
  fetchCompareData,
  isLoading,
  classes,
            ...
            {compareData.map((classData, idx) => {
              ...
              return (
                <div
                  ...
                  className={`${colors.light} rounded-xl p-5 border border-gray-200`}
                >
                  ...
                  <div className='space-y-3'>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <span
                        className={`font-bold ${
                          classData.period_total_change >= 0 ? 'text-green-600' : 'text-red-600'
                        }`}
                      >
                        {classData.period_total_change >= 0 ? '+' : ''}
                        {classData.period_total_change}
                      </span>
                    </div>
                    <div className='flex items-center justify-between'>
                      <span className='text-sm text-gray-500'>活跃学生</span>
                      <span className='font-semibold text-gray-800'>
                        {classData.period_active_students}
                      </span>
                    </div>
                  </div>
                </div>
                        ...
                        >
                          ...
                          <span
                            className={`font-semibold ${
                              student.current_score >= 0 ? 'text-green-600' : 'text-red-600'
                            }`}
                          >
