import type { ComponentType, CSSProperties, FormEvent, ChangeEvent } from 'react';
import {
  Plus,
  Search,
  Edit2,
...
interface RankRuleListViewProps {
  rankRules: RankRule[];
  isLoading: boolean;
  error: Error | null;
  searchTerm: string;
  ...
  sortedRules: RankRule[];
  editingRule: RankRule | null;
  onAddClick: () => void;
  onEditClick: (rule: RankRule) => void;
  handleDelete: (id: number) => Promise<void>;
  formData: FormData;
  setFormData: (data: Partial<FormData> | ((prev: FormData) => Partial<FormData>)) => void;
  formErrors: UseFormErrors<FormData>;
  setFormErrors: (errors: Partial<UseFormErrors<FormData>>) => void;
  showModal: boolean;
  closeModal: () => void;
  submitting: boolean;
...
export default function RankRuleListView({
  isLoading,
  error,
  searchTerm,
  setSearchTerm,
  ...
  handleSubmit,
  fetchRankRules,
}: RankRuleListViewProps) {
  const getIconComponent = (iconName: string): IconComponent => ICON_MAP[iconName] || Star;
        ...
        <div className='px-6 py-5 border-b border-gray-200/50 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4'>
          ...
          <div className='flex items-center gap-3'>
            <div className='flex items-center gap-2 px-4 py-2 bg-primary-50/80 rounded-xl'>
              ...
              <span className='text-sm font-semibold text-primary-600'>{filteredCount} 个等级</span>
            </div>
          </div>
        </div>
