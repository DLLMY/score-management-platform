...
const PhoneBoxPolicyInner: React.FC = () => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto space-y-6'>
      <WorkbenchBreadcrumb current='手机箱开箱策略' />
                ...
                {windows.map((w, idx) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={formatHourMinute(w.start_hour, w.start_minute)}
                      onChange={(e) => {
                    ...
                    <input
                      type='time'
                      aria-label={`结束时间（时段 ${idx + 1}）`}
                      className='px-3 py-2 border border-gray-300 rounded-lg bg-white dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200'
                      value={formatHourMinute(w.end_hour, w.end_minute)}
                      onChange={(e) => {
                        const { h, m } = parseTime(e.target.value);
