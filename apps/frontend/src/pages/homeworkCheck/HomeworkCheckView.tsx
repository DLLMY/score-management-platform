...
export type HomeworkFormErrors = Partial<Record<keyof HomeworkFormData, string>>;

export const defaultForm: HomeworkFormData = {
  id: null,
  class_id: 0,
...
const columns: ColumnType<HomeworkAssignment>[] = [
  {
    title: '作业标题',
    key: 'title',
    dataIndex: 'title',
...
const STATUS_TABS: ReadonlyArray<readonly [StatusFilter, string]> = [
  ['all', '全部'],
  ['pending', '进行中'],
  ['done', '已完成'],
];

interface HomeworkCheckViewProps {
  assignments: HomeworkAssignment[];
  loading: boolean;
  total: number;
  hwPage: number;
  setHwPage: (p: number) => void;
  searchTerm: string;
  ...
  setFormData: Dispatch<SetStateAction<HomeworkFormData>>;
  errors: HomeworkFormErrors;
  handleOpenModal: (isEdit?: boolean, assignment?: HomeworkAssignment) => void;
  handleDelete: (id: number) => void;
}

export default function HomeworkCheckView({
  assignments,
  loading,
  total,
  hwPage,
