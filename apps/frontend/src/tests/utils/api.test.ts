...
import { getErrorMessage, unwrapEnvelope, parseEnvelopeSafe } from '../../services/api';
...
describe('getErrorMessage (M2 信封归一化 · 友好文案优先级)', () => {
  ...
  it('后端 message 为业务文案（非技术报错）→ 透传给用户', () => {
    expect(getErrorMessage(400, { message: '积分不足，无法兑换' })).toBe('积分不足，无法兑换');
    expect(getErrorMessage(422, { error: '手机号格式不正确' })).toBe('手机号格式不正确');
  });

  it('后端 message 含技术细节（Traceback / 异常类 / SQL）→ 屏蔽，回退 HTTP 状态文案', () => {
    expect(getErrorMessage(500, { message: 'Traceback (most recent call last): ...' })).toBe(
      '服务器内部错误，请稍后重试'
    );
    expect(getErrorMessage(500, { message: 'RuntimeError: division by zero' })).toBe(
      '服务器内部错误，请稍后重试'
    );
    expect(getErrorMessage(500, { message: 'sqlalchemy.exc.OperationalError: ...' })).toBe(
      '服务器内部错误，请稍后重试'
    );
  });
...
describe('unwrapEnvelope (M2 信封解包)', () => {
  ...
  it('success=false（HTTP 2xx 但业务失败）→ 抛 ApiError(business, status=200)，避免调用方把失败当成功', () => {
    expect(() => unwrapEnvelope({ success: false, message: '余额不足', data: null })).toThrow();
  });
...
describe('parseEnvelopeSafe (M2 只读页安全解析 · 不抛)', () => {
  it('success=true → 返回 data', () => {
    expect(parseEnvelopeSafe<{ id: number }>({ success: true, data: { id: 1 } })).toEqual({
      id: 1,
    });
  });
