...
describe('useClientFilter', () => {
  test('returns items filtered by predicate', () => {
    const { result } = renderHook(() => useClientFilter([1, 2, 3, 4], (n) => n % 2 === 0, []));
  ...
  test('recomputes when deps change (predicate closes over dep)', () => {
    const { result, rerender } = renderHook(
      ({ keyword }: { keyword: string }) =>
        useClientFilter(['apple', 'banana', 'cherry'], (s) => s.includes(keyword), [keyword]),
  ...
  test('does not recompute when deps unchanged even if predicate recreated', () => {
    const items = [1, 2, 3];
    const { result, rerender } = renderHook(() => useClientFilter(items, (n) => n > 0, []));
