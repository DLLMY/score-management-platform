...
class InputValidator:
    ...
    def _numeric_error(self, value, bound_key, bound_val):
        """返回越界错误信息字符串，或 None（合法/不可比较）。"""
        try:
            val = int(value) if isinstance(value, (int, str)) else value
        except (ValueError, TypeError):
            return "必须是数字"
        if bound_key == "min" and val < bound_val:
            return f"不能小于 {bound_val}"
        if bound_key == "max" and val > bound_val:
            return f"不能大于 {bound_val}"
        return None

    def _apply_param_rule(self, field, value, rule):
        """参数化规则：min / max / minLength / maxLength。"""
        for key in ("min", "max"):
            if key in rule and value is not None:
                msg = self._numeric_error(value, key, rule[key])
                if msg:
    ...
    def _apply_simple_rule(self, field, value, rule):
        """简单字符串规则分发。"""
        if rule == "required":
        ...
        messages = {
            "email": "格式不正确",
            "phone": "格式不正确",
            "card_id": "格式不正确，需4-20位字母或数字",
            "username": "格式不正确，需字母开头，2-50位字母数字下划线",
            "password": "需至少6位，包含字母和数字",
            "integer": "必须是整数",
            "score": "必须是-10000到10000之间的整数",
            "class_name": "格式不正确",
            "gender": "只能是'男'或'女'",
            "datetime": "日期格式不正确",
            "json": "JSON格式不正确",
        ...
        if value and not validator(value):
            self.errors.append(f"{field} {messages[rule]}")
    ...
    def validate(self, field: str, value: Any, rules: list[dict[str, Any]]) -> bool:
        ...
        for rule in rules:
            if isinstance(rule, str):
                self._apply_simple_rule(field, value, rule)
            elif isinstance(rule, dict):
                self._apply_param_rule(field, value, rule)
