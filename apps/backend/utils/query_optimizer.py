...
def count_by_fk(model, fk_column, ids, extra_filter=None):
    """按外键分组计数（列表页「列表 + 关联 count」场景，单查询 group by）。

    收敛 subject_routes / categories_routes 等处的：
        dict(db.session.query(Model.fk, func.count(Model.id))
             .filter(Model.fk.in_(ids)[, extra]).group_by(Model.fk).all())

    :param model: 计数目标模型（如 ScoreRule）
    :param fk_column: 外键列（如 ScoreRule.category_id）
    :param ids: 外键值列表（空列表直接返回 {}）
    :param extra_filter: 可选额外过滤条件（如 ScoreRule.is_active.is_(True)）
    :return: {fk_id: count}
    """
    from sqlalchemy import func
    ...
    if not ids:
        return {}
    query = db.session.query(fk_column, func.count(model.id)).filter(fk_column.in_(ids))
    if extra_filter is not None:
        query = query.filter(extra_filter)
    return dict(query.group_by(fk_column).all())
