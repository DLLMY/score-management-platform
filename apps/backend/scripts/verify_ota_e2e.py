...
# ============================================================
def _setup_clients():
    """创建收/发包双连接（收发分离）并配置 TLS/认证，返回 (client, pub_client)。"""
    ...
    return client, pub_client


def _wait_ota_command():
    """等待设备收到 OTA 指令（最多 30s），返回最新指令元组或 None。"""
...
def _evaluate_command(cmd):
    """解析并验签收到的 OTA 指令，返回 (recv_ok, detail, sig_msg, pl)。"""
    topic, pl = cmd
    ...
    return recv_ok, detail, sig_msg, pl


def _report_status(pl):
    """设备回报 started -> success 状态（仅在验签通过时调用）。"""
    _publish(
...
def _wait_backend_closed(token):
    """等待后端闭环：ota-status 出现 completed 记录（最多 30s）。"""
    deadline = time.time() + 30
    while time.time() < deadline:
...
def main():
    ...
    client, pub_client = _setup_clients()
    ...
    # ---- 等待设备收到 OTA 指令 + 验签 ----
    results = []
    cmd = _wait_ota_command()
    if cmd is None:
        results.append(
            ("1.设备收到OTA指令", False, f"30s 内未在 phonebox/ota/{DEVICE_ID} 收到指令")
        )
    else:
        recv_ok, detail, sig_msg, pl = _evaluate_command(cmd)
    ...
    completed = _wait_backend_closed(token)
    results.append(
        (
