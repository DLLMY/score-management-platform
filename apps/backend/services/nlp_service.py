...
class NLPService:
    ...
    def _lookup_cache(self, text, parser_type, cache_key, redis_key):
        """按 内存->Redis 顺序查缓存；命中则标记 cached 并返回 copy，未命中返回 None。"""
        with self._cache_lock:
    ...
    def _try_fast_parser(self, text, parser_type, redis_key):
        """若启用 fast_parser 且无指定 parser_type，尝试快解析（置信度达标则缓存并返回）。"""
    ...
    def parse(self, text: str, parser_type: NLPParserType | None = None) -> dict[str, Any]:
        ...
        cached_result = self._lookup_cache(text, parser_type, cache_key, redis_key)
        if cached_result is not None:
            return cached_result

        self._cache_misses += 1

        fast_result = self._try_fast_parser(text, parser_type, redis_key)
        if fast_result is not None:
            return fast_result

        parser = self.get_parser(parser_type)
        if not parser:
            return self._unknown_result("解析器不可用")
