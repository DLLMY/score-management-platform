...
class TestGetRecordStatisticsView(unittest.TestCase):
    ...
    def test_permission_denied_when_class_not_allowed(self):
        ...
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=["ClassA"]),
    ...
    def test_date_parse_error_returns_400_bad_request(self):
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=None),
            mock.patch.object(
                self.srv, "parse_date_range", return_value=(None, None, "日期格式错误")
            ),
            mock.patch.object(self.srv, "get_score_statistics", return_value={}) as m_stat,
        ):
            view = self.srv.get_record_statistics_view(None, None, None, "bad", "bad")
        self.assertEqual(
            view, {"error": "日期格式错误", "status": 400, "error_code": "BAD_REQUEST"}
        )
    ...
    def test_normal_aggregates_with_admin_allowed_classes(self):
        ...
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=allowed),
            mock.patch.object(
                self.srv, "parse_date_range", return_value=("2026-01-01", "2026-02-01", None)
            ),
            mock.patch.object(
                self.srv, "get_score_statistics", return_value={"total": 5}
            ) as m_stat,
        ):
    ...
    def test_non_admin_passes_none_allowed_classes(self):
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=None) as m_allowed,
            mock.patch.object(self.srv, "parse_date_range", return_value=(None, None, None)),
...
class TestRecordListViews(unittest.TestCase):
    ...
    def test_list_normal_aggregates_with_allowed_classes(self):
        ...
        pag = _FakePagination(
            items=[types.SimpleNamespace(id=1)], total=10, page=2, per_page=50, pages=1
        )
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=["C1"]),
    ...
    def test_list_non_admin_passes_none_allowed_classes(self):
        ...
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=None) as m_a,
            mock.patch.object(self.srv, "parse_date_range", return_value=(None, None, None)),
            mock.patch.object(self.srv, "query_score_records", return_value=pag) as m_q,
    ...
    def test_by_user_normal_aggregates(self):
        pag = _FakePagination(
            items=[types.SimpleNamespace(id=9)], total=1, page=1, per_page=50, pages=1
        )
        with (
            mock.patch.object(self.srv, "can_access_student", return_value=True),
            mock.patch.object(self.srv, "query_score_records", return_value=pag) as m_q,
    ...
    def test_by_user_access_denied_returns_403(self):
        with (
            mock.patch.object(self.srv, "can_access_student", return_value=False),
            mock.patch.object(self.srv, "query_score_records", return_value=None) as m_q,
        ):
            view = self.srv.get_record_list_by_user_view(9, 1, 50)
    ...
    def test_score_entry_aggregates_with_allowed_classes(self):
        ...
        with (
            mock.patch.object(self.srv, "get_allowed_classes", return_value=["C1"]),
            mock.patch.object(
                self.srv, "get_score_entry_data", return_value={"rules": [], "users": []}
            ) as m_d,
        ):
