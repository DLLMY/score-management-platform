...
def test_train_model_without_samples_is_untrained(app):
    """P1-4：无训练样本时不得伪造 0.85 —— status=untrained 且 accuracy/f1 归零。

    与 P0-1（/model/evaluate 移除伪造 0.85）同一原则：规则侧自学习同样禁止假绿，
    没有样本就必须如实报告 untrained，让前端知道模型尚未训练。
    """
    service = NLPRuleManagementService()
    ...
    # 前端按 training_status 读取，须与 status 同步，否则永远显示"进行中"
    assert result["training_status"] == "untrained"


def test_train_model_computes_real_accuracy_from_samples(app):
    """P1-4：有样本时按真实标注计算准确率（未被人工纠正的占比），落库 completed。"""
    from models import NLPMatchResult, db
    ...
    with app.app_context():
        ...
        result = NLPRuleManagementService().train_model()

    assert result["status"] == "completed"
    assert result["training_data_size"] == 4
