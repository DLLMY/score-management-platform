...
class TestWOLRoutes:
    ...
    def test_create_wol_device_duplicate_mac(self, client, app, auth_headers, db_session):
        with app.app_context():
            dev = Device(
                device_id="wol-AA:BB:CC:DD:EE:02",
                name="已有",
                device_type="wol",
            ...
            response = client.post(
                "/api/wol/devices",
                json={"name": "新", "mac_address": "aa:bb:cc:dd:ee:02"},
    ...
    def test_update_wol_device_success(self, client, app, auth_headers, db_session):
        with app.app_context():
            dev = Device(
                device_id="wol-AA:BB:CC:DD:EE:03",
                name="旧名",
                device_type="wol",
    ...
    def test_update_wol_device_not_found(self, client, app, auth_headers):
        with app.app_context():
            response = client.put(
                "/api/wol/devices/99999", json={"name": "x"}, headers=auth_headers
            )
    ...
    def test_update_wol_device_duplicate_mac(self, client, app, auth_headers, db_session):
        with app.app_context():
            d1 = Device(
                device_id="wol-AA:BB:CC:DD:EE:10",
                name="d1",
                device_type="wol",
            ...
            d2 = Device(
                device_id="wol-AA:BB:CC:DD:EE:11",
                name="d2",
                device_type="wol",
            ...
            db_session.commit()
            response = client.put(
                f"/api/wol/devices/{d2.id}",
                json={"mac_address": "aa:bb:cc:dd:ee:10"},
                headers=auth_headers,
            )
            assert response.status_code == 409
    ...
    def test_delete_wol_device_success(self, client, app, auth_headers, db_session):
        with app.app_context():
            dev = Device(
                device_id="wol-AA:BB:CC:DD:EE:20",
                name="待删",
                device_type="wol",
