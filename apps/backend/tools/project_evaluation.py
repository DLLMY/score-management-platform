        ...
        if os.path.exists(cf):
            try:
                ...
                if secret_match:
                    ...
                    jwt_config["secret_length"] = len(secret)
                    jwt_config["has_hardcoded"] = True
                    if len(secret) < 32:
                        jwt_config["risk"] = "JWT密钥长度不足32字节"
                # 检查是否使用环境变量
                env_match = re.search(r'os\.environ\.get\([\'"]JWT_SECRET_KEY[\'"]', content)
                jwt_config["uses_env_var"] = bool(env_match)
            except Exception as e:
                logger.warning("项目评估采集异常（已跳过该项）: %s", e)
    return jwt_config


def _check_csrf_config():
    """检查 CSRF 保护是否启用。"""
    csrf_config = {}
    try:
        with open(os.path.join(BACKEND_DIR, "app", "config.py"), encoding="utf-8") as f:
            content = f.read()
        csrf_config["enabled"] = "CSRF" in content and "True" in content
    except Exception:
        csrf_config["enabled"] = "unknown"
    return csrf_config


def _check_permission_system():
    """扫描 api 目录的 @requires_permission 装饰器数量。"""
    perm_decorators = 0
    perm_files_checked = []
        ...
        for fname in files:
            if fname.endswith(".py"):
                ...
                try:
                    ...
                    logger.warning("项目评估采集异常（已跳过该项）: %s", e)
    return {
        "total_decorators": perm_decorators,
        "files_with_permissions": len(perm_files_checked),
    }


def _check_hardcoded_secrets():
    """扫描 services 目录的硬编码密码/密钥风险。"""
    hardcoded_secrets = []
    patterns_to_check = [
...
def _check_sql_injection_risks():
    """扫描全仓字符串拼接的 SQL（execute/raw + f-string）风险。"""
    sql_injection_risks = []
    for root, dirs, files in os.walk(BACKEND_DIR):
...
def collect_security_metrics():
    ...
    sec_metrics = {
        ...
        "csrf_config": _check_csrf_config(),
        "permission_system": _check_permission_system(),
        "password_security": {},
        "dependency_issues": [],
        "hardcoded_secrets": _check_hardcoded_secrets(),
        "sql_injection_risks": _check_sql_injection_risks(),
    }
    perm_decorators = sec_metrics["permission_system"]["total_decorators"]
    # 打印结果
