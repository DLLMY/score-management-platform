...
type EmptyStateIcon = 'users' | 'data' | 'search' | 'file' | 'folder' | 'wifi' | 'bell' | 'settings' | 'alert';
...
const SearchEmptyState = memo<SearchEmptyStateProps>(({ searchTerm = '', onClearSearch = null, className = '' }) => {
  return (
    <div
      className={`flex flex-col items-center justify-center py-16 px-6 text-center ${className}`}
    >
...
const ErrorState = memo<ErrorStateProps>(({ message = '加载失败', onRetry = null, className = '' }) => {
  return (
    <div
      className={`flex flex-col items-center justify-center py-16 px-6 text-center ${className}`}
    >
