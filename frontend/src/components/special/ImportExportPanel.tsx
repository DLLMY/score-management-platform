...
function ImportExportPanel({
  ...
  const handleExport = async (format: 'excel' | 'csv' = 'excel') => {
    ...
    try {
      if (onDataExport) {
        ...
        downloadBlob(
          blob,
          `${type}_${format}_${Date.now()}.${format === 'excel' ? 'xlsx' : 'csv'}`
        );
  ...
  const handleDownloadFailedData = () => {
    ...
    const lines = reImportData.map((msg, idx) => [
      ...
      ...keyList.map((k) =>
        escapeCsv(msg.row_data ? (msg.row_data as Record<string, unknown>)[k] : undefined)
      ),
    ]);
    const csv = '\ufeff' + [headers.join(','), ...lines.map((row) => row.join(','))].join('\n');
    downloadTextAsFile(csv, `导入失败数据_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className='space-y-3'>
      ...
      <Modal isOpen={isImportModalOpen} onClose={resetImportState} title={`导入${typeLabel}数据`}>
        <div className='space-y-4'>
          ...
          <div
            className='border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-primary-400 hover:bg-primary-50/30 transition-colors cursor-pointer'
          ...
          {importResult && (
            ...
            >
              <div className='flex items-center justify-between mb-3'>
                <div className='flex items-center gap-2'>
                  {importResult.success ? (
                    ...
                    <AlertCircle
                      className={`w-5 h-5 ${
                        importResult.failed_count ? 'text-amber-500' : 'text-red-500'
                      }`}
                    />
                  )}
                  <span
              ...
              {(importResult.total !== undefined ||
                ...
                importResult.failed_count !== undefined) && (
              ...
              <div className='max-h-40 overflow-y-auto space-y-1 mb-3'>
                {reImportData.slice(0, 20).map((msg, idx) => (
                  <div key={idx} className='text-xs p-2 bg-white rounded border border-amber-100'>
                    <span className='text-red-500 font-medium'>行{msg.row_number || idx + 1}:</span>
                    <span className='text-gray-600 ml-1'>{msg.message}</span>
                    {msg.row_data && (
                      <div className='mt-1 text-gray-400 truncate'>
                        {Object.entries(msg.row_data)
                          .map(([k, v]) => `${k}:${v}`)
                          .join(' | ')}
                      </div>
                    )}
          ...
          <div className='flex gap-2 pt-2'>
            <Button onClick={resetImportState} variant='outline' className='flex-1'>
              {importResult ? '关闭' : '取消'}
            </Button>
            <Button
