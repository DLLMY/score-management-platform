...
const PieChartComponent = ({ data, title = '占比分布', dataKey = 'value', nameKey = 'name', innerRadius = 0 }) => {
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const data = payload[0];
      const total = data.payload.total || data.payload[nameKey]?.reduce((sum, d) => sum + d.value, 0) || 0;
