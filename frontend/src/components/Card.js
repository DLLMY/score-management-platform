...
function Card({ 
  ...
  return (
    ...
    `}>
      {title && (
        <div className={`px-5 py-4 border-b ${variant === 'dark' ? 'border-slate-700/50 bg-slate-700/20' : 'border-gray-100 bg-gray-50/50'} ${headerClass}`}>
          {Icon && (
      ...
      <div className={`p-5 ${variant === 'dark' ? 'text-slate-300' : ''}`}>
        {children}
      </div>
    </div>
  );
