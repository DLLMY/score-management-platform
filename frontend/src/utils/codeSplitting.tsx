...
export const createRouteComponent = <P extends {} = {}>(
  ...
  const WrappedComponent: React.FC<P> = (props) => {
    // 泛型 lazy 透传：TS 对 LazyExoticComponent<P> 的 JSX 泛型 props 推导存在已知限制，
    // 经 unknown 边界转换为 React.FC<P> 以收敛 any。
    const C = LazyComponent as unknown as React.FC<P>;
    return (
...
export const createLazyComponent = <P extends {} = {}>(
  ...
  const WrappedComponent: React.FC<P & { fallback?: ReactNode }> = (props) => {
    ...
    const C = LazyComponent as unknown as React.FC<P & { fallback?: ReactNode }>;
    return (
      <Suspense fallback={fallback || defaultFallback}>
        <C {...(restProps as P)} />
      </Suspense>
