...
describe('RequestCoalescer', () => {
  ...
  test('should handle rejected promises', async () => {
    ...
    await expect(coalescer.coalesce({ url: '/api/error', method: 'GET' }, fetcher)).rejects.toThrow(
      error
    );
    expect(fetcher).toHaveBeenCalledTimes(1);
  });
  ...
  test('should handle params in request key', async () => {
    ...
    await coalescer.coalesce(
      { url: '/api/search', method: 'GET', params: { q: 'other' } },
      fetcher
    );
  ...
  test('should handle body in request key', async () => {
    ...
    await coalescer.coalesce(
      { url: '/api/create', method: 'POST', body: { name: 'test' } },
      fetcher
    );
    await coalescer.coalesce(
      { url: '/api/create', method: 'POST', body: { name: 'test' } },
    ...
    await coalescer.coalesce(
      { url: '/api/create', method: 'POST', body: { name: 'other' } },
      fetcher
    );
