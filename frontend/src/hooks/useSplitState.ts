...
export function useSplitState<T extends Record<string, unknown>>(
  ...
  const batchUpdate = useCallback(
    (volatileUpdate: Partial<T>, stableUpdate: Partial<T>, delay: number = 100) => {
      // 立即更新 volatile 状态
 ...
 */
export function useSmartSplitState<T extends Record<string, unknown>>(initialState: T) {
