          ...
          try {
            ...
            if (msg.box_id && msg.status) {
              ...
              const normalizedStatus =
                msg.status === 'open' ? 'opened' : msg.status === 'close' ? 'closed' : msg.status;
        ...
        if (log.topic === 'phonebox/heartbeat' && log.direction === 'receive') {
          try {
            ...
            if (msg.box_a_status) {
              newBoxStatus['A'] = {
                status:
                  msg.box_a_status === 'open'
                    ? 'opened'
                    : msg.box_a_status === 'closed'
                      ? 'closed'
                      : msg.box_a_status,
                lastUpdate: log.timestamp,
              };
            ...
            if (msg.box_b_status) {
              newBoxStatus['B'] = {
                status:
                  msg.box_b_status === 'open'
                    ? 'opened'
                    : msg.box_b_status === 'closed'
                      ? 'closed'
                      : msg.box_b_status,
                lastUpdate: log.timestamp,
              };
    ...
    async (card) => {
      ...
      try {
        ...
        if (card.status === 'success') {
          ...
          addLog(
            `验证通过: ${card.reason} -> phonebox/unlock/B (设备: ${selectedDevice.device_id})`,
            'send'
          );
      ...
      <div className='max-w-5xl mx-auto px-4 py-6'>
        <header className='mb-6'>
          <div className='flex items-center justify-between'>
            ...
            >
              <div
                className={`w-2 h-2 rounded-full ${mqttStatus ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}
              />
            ...
            {activeTab === 'subscribe' && (
              <div className='space-y-6'>
                ...
                <div className='p-4 bg-gray-50 rounded-xl'>
                  ...
                  <div className='flex flex-wrap gap-2'>
                    {['phonebox/status', 'phonebox/log', 'phonebox/query', 'phonebox/unlock/+'].map(
                      (topic) => (
                        <button
                          key={topic}
                    ...
                    filteredLogs.map((log) => {
                      ...
                      try {
                        ...
                        if (parsed.result && parsed.reason) {
                          ...
                          const reasonText =
                            parsed.reason === 'score_ok'
                              ? '积分充足'
                              : parsed.reason === 'score_low'
                                ? '积分不足'
                                : parsed.reason === 'card_not_found'
                                  ? '卡号未注册'
                                  : parsed.reason;
                      ...
                      return (
                        ...
                        >
                          ...
                          {log.topic && <span className='text-blue-400 text-xs'>{log.topic}</span>}
              ...
              <div className='space-y-6'>
                <div className='bg-gradient-to-r from-primary-500 to-indigo-600 rounded-xl p-4 text-white'>
                  <div className='flex items-center justify-between'>
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full ${mqttStatus ? 'bg-green-400 animate-pulse' : 'bg-red-400'}`}
                      />
                      {mqttStatus ? '已连接' : '未连接'}
                    </div>
                  </div>
                    ...
                    <div className='mt-3 pt-3 border-t border-gray-100'>
                      <div className='grid grid-cols-2 md:grid-cols-4 gap-4 text-sm'>
                        ...
                        <div>
                          ...
                          <p
                            className={`font-medium ${selectedDevice.status === 'online' ? 'text-green-600' : 'text-red-600'}`}
                          >
                        ...
                        <div>
                          ...
                          <p className='text-gray-900'>
                            {formatDateTime(selectedDevice.last_heartbeat)}
                          </p>
                        </div>
                      </div>
                    </div>
