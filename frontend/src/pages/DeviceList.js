...
function DeviceList() {
  ...
  const loadDevices = async (manualRefresh = false) => {
    if (manualRefresh) {
        ...
        <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{stats.total_devices || 0}</p>
            </div>
        ...
        <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{stats.online_devices || 0}</p>
            </div>
            <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center">
        ...
        <Card className="bg-gradient-to-br from-red-500 to-red-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{stats.offline_devices || 0}</p>
            </div>
            <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center">
        ...
        <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{stats.today_heartbeats || 0}</p>
            </div>
            <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center">
        ...
        {initialLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {devices.map((device) => {
                  const signal = getSignalStrength(device.wifi_signal);
                  return (
                    <tr key={device.device_id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className={`w-6 h-2 rounded-full ${signal.color}`} />
                          <span className="text-sm">{signal.text}</span>
                          {device.wifi_signal && <span className="text-xs text-gray-400">({device.wifi_signal} dBm)</span>}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm">{formatUptime(device.uptime)}</td>
                      <td className="px-4 py-3">
                        <Badge 
                          variant={device.box_a_status === 'opened' ? 'warning' : 'success'}
                        >
                      ...
                      <td className="px-4 py-3">
                        <Badge 
                          variant={device.box_b_status === 'opened' ? 'warning' : 'success'}
                        >
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <p className={`text-lg font-medium ${selectedDevice.box_a_status === 'opened' ? 'text-yellow-600' : 'text-green-600'}`}>
                  {selectedDevice.box_a_status === 'opened' ? '打开' : '关闭'}
                </p>
              </div>
              <div className="p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-500">B箱状态</p>
                <p className={`text-lg font-medium ${selectedDevice.box_b_status === 'opened' ? 'text-yellow-600' : 'text-green-600'}`}>
