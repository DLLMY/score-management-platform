...
const getPeriodType = (startHour: number) => {
  if (startHour < 12) return { label: '上午', color: 'text-amber-600 bg-amber-50 border-amber-200', icon: Sun };
  if (startHour < 17) return { label: '下午', color: 'text-blue-600 bg-blue-50 border-blue-200', icon: Sun };
...
const ClassPeriodSettings: React.FC = () => {
  ...
  const totalDuration = periods
    ...
    .reduce((sum, p) => sum + (((parseInt(String(p.end_hour), 10) || 0) * 60 + (parseInt(String(p.end_minute), 10) || 0)) - 
                                ((parseInt(String(p.start_hour), 10) || 0) * 60 + (parseInt(String(p.start_minute), 10) || 0))), 0);
        ...
        <div className='bg-white dark:bg-slate-800 rounded-2xl p-5 border border-gray-100 dark:border-slate-700 shadow-sm'>
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-3xl font-bold text-gray-800 dark:text-white mt-1'>{periods.length}</p>
            </div>
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-3xl font-bold text-primary-600 mt-1'>
                {Math.floor(totalDuration / 60)}<span className='text-lg'>小时</span>{totalDuration % 60}<span className='text-lg'>分</span>
              </p>
            </div>
      ...
      <div className='bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden'>
        ...
        {periods.length === 0 ? (
          ...
          <div className='divide-y divide-gray-100 dark:divide-slate-700'>
            {periods
              .sort((a, b) => (parseInt(String(a.sort_order), 10) || 0) - (parseInt(String(b.sort_order), 10) || 0))
              .map((period, index) => {
                const typeInfo = getPeriodType(parseInt(String(period.start_hour), 10) || 0);
                const TypeIcon = typeInfo.icon;
                const duration =
                  ((parseInt(String(period.end_hour), 10) || 0) * 60 + (parseInt(String(period.end_minute), 10) || 0)) -
                  ((parseInt(String(period.start_hour), 10) || 0) * 60 + (parseInt(String(period.start_minute), 10) || 0));
                return (
                  <div
                    key={period.id}
                  ...
                  >
                    ...
                    {/* 类型标签 */}
                    <div className={`px-2 py-1 rounded-lg text-xs font-medium border shrink-0 flex items-center gap-1 ${typeInfo.color}`}>
                    ...
                    <div className='flex-1 min-w-0'>
                      <div className='flex items-center gap-2'>
                        <span className='font-semibold text-gray-800 dark:text-white'>{period.name}</span>
                        {!period.is_active && (
                  ...
                  >
                    {period.is_active ? <Check className='w-4 h-4' /> : <X className='w-4 h-4' />}
                  </PermissionButton>
                  <PermissionButton
                    permission='timetable.rule.manage'
              ...
              <div className='flex items-center justify-between'>
                <div className='flex items-center gap-3'>
                  <div className='w-10 h-10 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-xl flex items-center justify-center'>
                    {editingPeriod ? <Edit2 className='w-5 h-5 text-white' /> : <Plus className='w-5 h-5 text-white' />}
                  </div>
                  <div>
            ...
            <form onSubmit={handleSubmit} className='p-6 space-y-5'>
              ...
              <div className='flex items-center gap-3'>
                <label className='flex items-center gap-3 cursor-pointer'>
                  ...
                  />
                  <span className='text-sm font-medium text-gray-700 dark:text-gray-300'>启用此节次</span>
                </label>
              </div>
