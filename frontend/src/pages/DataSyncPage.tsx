...
const DataSyncPage: React.FC = () => {
  ...
  const fetchCheck = useCallback(async () => {
    try {
      ...
      const result = (await request('/api/consistency/check')) as {
    ...
    <div className='space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-4 mb-6'>
          <div className='bg-white rounded-xl p-4 shadow-sm border'>
            ...
            <div className='mt-2'>
              ...
              <div className='text-xs text-gray-400 mt-1'>
                {stats.users.link_rate ? stats.users.link_rate : '--'}
              </div>
            </div>
          </div>
          ...
          <div className='bg-white rounded-xl p-4 shadow-sm border'>
            ...
            <div className='mt-2'>
              ...
              <div className='text-xs text-gray-400 mt-1'>
                {stats.admins.link_rate ? stats.admins.link_rate : '--'}
              </div>
            </div>
          </div>
          ...
          <div className='bg-white rounded-xl p-4 shadow-sm border'>
            <div className='flex items-center gap-3'>
              <AlertTriangle
                className={`w-8 h-8 ${issues.length > 0 ? 'text-yellow-500' : 'text-green-500'}`}
              />
              <div>
      ...
      {stats && stats.missing_classes && stats.missing_classes.length > 0 && (
        <div className='bg-orange-50 rounded-xl p-4 mb-6 border border-orange-200'>
          ...
          <p className='text-sm text-orange-600 mt-2'>点击"执行修复"将自动创建这些班级并建立关联</p>
        </div>
      )}
      ...
      <div className='mt-6 bg-gray-50 rounded-xl p-4 border'>
        ...
        <ul className='text-sm text-gray-600 space-y-1'>
          ...
          <li>
            • 管理员 (Admin) 的 class_name 字段现在通过 primary_class_id 外键关联到 ClassInfo 表
          </li>
