...
function Analysis() {
  ...
  const filteredUsers = selectedClass 
    ? users.filter(u => u.class_name === selectedClass)
    : users;
  ...
  const scoreDistribution = [
    { name: '0-59', count: filteredUsers.filter(u => u.current_score < 60).length, color: '#ef4444' },
    { name: '60-79', count: filteredUsers.filter(u => u.current_score >= 60 && u.current_score < 80).length, color: '#f59e0b' },
    { name: '80-100', count: filteredUsers.filter(u => u.current_score >= 80).length, color: '#22c55e' }
  ];
                        ...
                        <div className="progress-bar">
                          <div 
                            className={`progress-fill ${
                              stats.qualifiedRate >= 80 ? 'bg-success-500' :
                              stats.qualifiedRate >= 50 ? 'bg-warning-500' : 'bg-danger-500'
                            }`}
              ...
              <div className="card-body">
                <div className="space-y-4">
                  {topUsers.map((user, index) => (
                    <div key={user.id} className={`flex items-center gap-4 p-4 rounded-xl transition-all ${
                      ...
                      <div className={`text-right ${
                        user.current_score >= 80 ? 'text-success-600' :
                        user.current_score >= 60 ? 'text-primary-600' : 'text-danger-600'
                      }`}>
              ...
              {needAttention.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {needAttention.map(user => (
                    <div key={user.id} className="bg-gradient-to-r from-danger-50 to-orange-50 border border-danger-100 rounded-xl p-4">
                      <div className="flex items-center gap-3">
