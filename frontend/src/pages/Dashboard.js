...
import { 
  Trophy, Smartphone, Bell, TrendingUp, Clock, Users, ArrowUp, ArrowDown, 
  Zap, Activity, Eye, RefreshCw, ChevronDown, Building2, Award, Target, Flame, 
  Star, Crown, BarChart3, LineChart, PieChart, AlertTriangle, CheckCircle, XCircle,
...
const AnimatedNumber = memo(({ value, className = '', decimals = 0 }) => {
  ...
  return <span className={className}>{displayValue.toLocaleString(undefined, { maximumFractionDigits: decimals })}</span>;
});

const StatCard = memo(({ icon: Icon, label, value, subValue, trend, color, gradient, description, delay = 0 }) => {
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br ${gradient}`} style={{ opacity: 0.05 }} />
      <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${gradient}`} />
      
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <div className={`relative w-12 h-12 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg transition-all duration-300 ${isHovered ? 'scale-115 shadow-xl' : ''}`}>
            <Icon className={`w-6 h-6 text-white transition-transform duration-300 ${isHovered ? 'scale-110' : ''}`} />
            <div className={`absolute inset-0 rounded-xl bg-gradient-to-br ${gradient} opacity-0 ${isHovered ? 'opacity-50' : ''} blur-2xl transition-opacity duration-500`} />
          </div>
          {trend && (
            <div className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold transition-all duration-300 ${trend > 0 ? 'bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-green-500 border border-green-500/30' : 'bg-gradient-to-r from-red-500/20 to-rose-500/20 text-red-500 border border-red-500/30'} ${isHovered ? 'scale-110' : ''}`}>
              {trend > 0 ? <ArrowUp className="w-3 h-3" /> : <TrendingDownIcon className="w-3 h-3" />}
              {Math.abs(trend)}%
            </div>
          )}
        </div>
      ...
      <div className={`absolute -bottom-2 -right-2 w-24 h-24 bg-gradient-to-br from-gray-100/50 to-transparent rounded-full blur-3xl transition-all duration-500 ${isHovered ? 'opacity-100 scale-125' : 'opacity-0'}`} />
...
function Dashboard() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      return Array.isArray(notificationsData) ? notificationsData : notificationsData.notifications || [];
    } catch (e) {
  ...
  const fetchData = useCallback(async (manualRefresh = false) => {
    ...
    try {
      ...
      const hasUserScoreChanges = prevUsers.length > 0 && sortedUsers.some(user => {
      ...
      if (currentState.records.length === 0 || recordsList[0]?.id !== currentState.records[0]?.id) {
        dispatch({ type: 'SET_RECORDS', payload: recordsList });
      }

      const hasDeviceChanges = deviceList !== null && (
      ...
      if (hasDeviceChanges) {
        ...
        const mockDevices = [
          { id: 1, device_id: 'phonebox_001', status: 'online', is_online: true, last_heartbeat: new Date().toISOString(), wifi_signal: -45, uptime: 9651, battery_level: 85 },
          { id: 2, device_id: 'phonebox_002', status: 'online', is_online: true, last_heartbeat: new Date().toISOString(), wifi_signal: -52, uptime: 8234, battery_level: 72 },
          { id: 3, device_id: 'phonebox_003', status: 'offline', is_online: false, last_heartbeat: new Date(Date.now() - 3600000).toISOString(), wifi_signal: -80, uptime: 0, battery_level: 0 }
        ];
      ...
      const hasNotificationChanges = notificationsList !== null && (
        currentState.notifications.length === 0 || 
        notificationsList.some(n => !currentState.notifications.find(on => on.id === n.id))
      );
      ...
      if (hasNotificationChanges) {
        ...
        dispatch({ type: 'SET_NOTIFICATIONS', payload: [
          { id: 1, title: '系统通知', message: '积分管理系统更新完成', created_at: new Date().toISOString(), read: false, type: 'info' },
          { id: 2, title: '提醒', message: '请及时审核学生加分申请', created_at: new Date(Date.now() - 3600000).toISOString(), read: false, type: 'warning' },
          { id: 3, title: '设备告警', message: '设备phonebox_003离线超过1小时', created_at: new Date(Date.now() - 7200000).toISOString(), read: false, type: 'error' },
          { id: 4, title: '数据统计', message: '今日积分发放已完成，共发放1250积分', created_at: new Date(Date.now() - 1800000).toISOString(), read: true, type: 'success' }
        ]});
      }
      ...
      const newStatistics = {
        ...
        onlineDevices: deviceList !== null ? getOnlineCount(deviceList) : currentState.statistics.onlineDevices
      };
  ...
  const getLevel = (score) => {
    if (score >= 95) return { text: '领航者', icon: '🏆', color: 'from-yellow-500/40 to-amber-500/30 text-yellow-400 border-yellow-500/50' };
    if (score >= 85) return { text: '自律星', icon: '⭐', color: 'from-blue-500/40 to-cyan-500/30 text-blue-400 border-blue-500/50' };
    if (score >= 75) return { text: '进取者', icon: '🚀', color: 'from-green-500/40 to-emerald-500/30 text-green-400 border-green-500/50' };
    if (score >= 65) return { text: '稳定区', icon: '📊', color: 'from-teal-500/40 to-cyan-500/30 text-teal-400 border-teal-500/50' };
    if (score >= 60) return { text: '安全基准', icon: '✅', color: 'from-cyan-500/40 to-blue-500/30 text-cyan-400 border-cyan-500/50' };
    if (score >= 50) return { text: '浅观察', icon: '⚠️', color: 'from-orange-500/40 to-amber-500/30 text-orange-400 border-orange-500/50' };
    if (score >= 40) return { text: '深观察', icon: '🔴', color: 'from-red-500/40 to-rose-500/30 text-red-400 border-red-500/50' };
    if (score >= 30) return { text: '限行区', icon: '🚨', color: 'from-pink-500/40 to-rose-500/30 text-pink-400 border-pink-500/50' };
    if (score >= 20) return { text: '重启预备', icon: '🔄', color: 'from-purple-500/40 to-violet-500/30 text-purple-400 border-purple-500/50' };
    if (score >= 10) return { text: '护航区', icon: '🛡️', color: 'from-indigo-500/40 to-purple-500/30 text-indigo-400 border-indigo-500/50' };
    return { text: '重生点', icon: '💀', color: 'from-gray-500/40 to-slate-500/30 text-gray-400 border-gray-500/50' };
  };
      ...
      >
        ...
        {isTopThree && (
          <div className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-gradient-to-br from-yellow-400 via-amber-500 to-orange-500 flex items-center justify-center shadow-lg shadow-yellow-500/40 animate-fade-in">
            {globalIndex === 0 ? <Crown className="w-3.5 h-3.5 text-white" /> : 
             globalIndex === 1 ? <Award className="w-3.5 h-3.5 text-white" /> : 
        ...
        <div className="flex items-center gap-2.5">
          {/* 排名头像 */}
          <div className={`relative w-11 h-11 rounded-full bg-gradient-to-br ${getRankColor(globalIndex)} flex items-center justify-center shadow-md overflow-hidden transition-all duration-300 ${isHovered ? 'scale-115 rotate-3' : ''}`}>
            {globalIndex < 3 ? (
              <span className="text-xl">{globalIndex === 0 ? '🥇' : globalIndex === 1 ? '🥈' : '🥉'}</span>
            ) : (
              <span className="text-sm font-bold text-white">{globalIndex + 1}</span>
            )}
            <div className={`absolute inset-0 bg-gradient-to-br ${getRankColor(globalIndex)} opacity-0 ${isHovered ? 'opacity-50' : ''} blur-xl transition-opacity duration-300`} />
          </div>
          
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
              <p className="font-semibold text-gray-900 text-sm truncate">{user.name}</p>
              <span className={`text-lg font-bold ${getScoreColor(score)} flex items-center gap-0.5 transition-all duration-300 ${isHovered ? 'scale-110' : ''}`}>
                {score}
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full bg-gradient-to-r ${level.color} flex items-center gap-1 transition-all duration-300 ${isHovered ? 'scale-110 px-3' : ''}`}>
        ...
        {/* 悬停光晕效果 */}
        <div className={`absolute inset-0 bg-gradient-to-br from-blue-500/5 to-purple-500/5 rounded-xl transition-all duration-300 ${isHovered ? 'opacity-100 scale-102' : 'opacity-0'}`} />
        
        {/* 边框动画 */}
        <div className={`absolute inset-0 border-2 border-blue-300/30 rounded-xl transition-all duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`} />
      </div>
    );
  };
      ...
      >
        ...
        <div className="flex items-center gap-2.5">
          <div className={`relative w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-300 ${
            isOnline ? 'bg-gradient-to-br from-green-500 to-emerald-500 shadow-lg shadow-green-500/40' : 'bg-gradient-to-br from-red-500 to-rose-500 shadow-lg shadow-red-500/40'
          } ${isHovered ? 'scale-115' : ''}`}>
            <Smartphone className={`w-5.5 h-5.5 text-white transition-transform duration-300 ${isHovered ? 'scale-110' : ''}`} />
            {isOnline && (
              <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 rounded-full border-2 border-white animate-pulse" />
            )}
          </div>
          <div className="flex-1 min-w-0">
            <h4 className="font-semibold text-gray-900 text-sm truncate">{device.device_id || device.name || `设备 ${device.id}`}</h4>
            <div className="flex items-center justify-between mt-1">
              <span className={`text-xs font-semibold flex items-center gap-1.5 ${isOnline ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <div className="flex-1 h-1.5 bg-gray-200/80 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-700 ${
                      device.battery_level > 50 ? 'bg-gradient-to-r from-green-500 to-emerald-400' : 
                      device.battery_level > 20 ? 'bg-gradient-to-r from-yellow-500 to-amber-400' : 'bg-gradient-to-r from-red-500 to-rose-400'
                    }`} 
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className={`relative w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-300 ${
              isPositive ? 'bg-gradient-to-br from-green-500 to-emerald-500 shadow-lg shadow-green-500/40' : 'bg-gradient-to-br from-red-500 to-rose-500 shadow-lg shadow-red-500/40'
            } ${isHovered ? 'scale-115' : ''}`}>
              {isPositive ? <ArrowUp className={`w-4.5 h-4.5 text-white transition-transform duration-300 ${isHovered ? '-translate-y-0.5' : ''}`} /> : <ArrowDown className={`w-4.5 h-4.5 text-white transition-transform duration-300 ${isHovered ? 'translate-y-0.5' : ''}`} />}
            </div>
            <div className="min-w-0">
              <h4 className="font-semibold text-gray-900 text-sm truncate">{record.user_name || record.student_name || '-'}</h4>
              {record.rule_name && (
          ...
          <div className="text-right">
            <span className={`text-xl font-bold transition-all duration-300 ${isPositive ? 'text-green-600' : 'text-red-600'} ${isHovered ? 'scale-110' : ''}`}>
              {isPositive ? '+' : ''}{record.score_change || 0}
    ...
    return (
      ...
      >
        ...
        <div className="flex items-start gap-3">
          <div className={`relative w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
            notification.read 
              ? 'bg-gray-200/50' 
              : `bg-gradient-to-br ${gradient} shadow-lg`
          } ${isHovered ? 'scale-115' : ''}`}>
            <Icon className={`w-4.5 h-4.5 transition-transform duration-300 ${notification.read ? 'text-gray-500' : 'text-white'} ${isHovered ? 'scale-110' : ''}`} />
          </div>
          <div className="flex-1 min-w-0">
            <h4 className={`font-semibold text-sm mb-1 transition-colors duration-300 ${notification.read ? 'text-gray-700' : 'text-gray-900'}`}>
              {notification.title}
            </h4>
        ...
        {data.map((value, index) => (
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <span className={`text-xs font-semibold transition-colors duration-300 ${hoveredIndex === index ? 'text-blue-500' : 'text-gray-600'}`}>
                {value}
              </span>
              <span className="text-xs text-gray-500 text-center truncate max-w-full">{labels[index] || ''}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    return (
      <div className="flex flex-col items-center">
        <svg className="w-36 h-36" viewBox="0 0 100 100">
          ...
          <text x="50" y="47" textAnchor="middle" fill="#94a3b8" fontSize="8">总计</text>
          <text x="50" y="58" textAnchor="middle" fill="#1e293b" fontSize="11" fontWeight="bold">{total}人</text>
        </svg>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-3 md:p-4">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-purple-500/6 rounded-full blur-3xl animate-pulse-slow" style={{ animationDelay: '1s' }} />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-3">
        ...
        <div className="flex-1 bg-white border border-gray-200/50 rounded-2xl p-4">
          ...
          <div ref={scrollContainerRef} className="overflow-y-auto rounded-xl" style={{ maxHeight: '400px', scrollbarWidth: 'thin', scrollbarColor: '#94a3b8 #e2e8f0' }}>
            {state.loading ? (
                ...
                {classGroups.map((group, groupIndex) => (
                  <div key={group.class_name} className="space-y-2">
                    <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-gray-100 via-gray-50 to-gray-100 rounded-xl border border-gray-200/50 sticky top-0 z-10">
                      ...
                      <span className="text-sm font-semibold text-gray-900">{group.class_name}</span>
          ...
          <div className="bg-white border border-gray-200/50 rounded-2xl p-4 flex-1">
            ...
            <div className="space-y-2">
              {state.loading ? (
                ...
                state.devices.slice(0, 3).map((device) => <DeviceCard key={device.id} device={device} />)
              ) : (
          ...
          <div className="bg-white border border-gray-200/50 rounded-2xl p-4 flex-1">
            ...
            <div className="space-y-2">
              {state.loading ? (
                ...
                state.records.slice(0, 4).map((record) => <RecordItem key={record.id} record={record} />)
              ) : (
                <div className="text-center py-4">
