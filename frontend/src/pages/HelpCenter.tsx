  ...
  const faqs: Faq[] = [
    {
      question: '如何添加新学生？',
      answer:
        '在左侧导航栏点击「学生管理」，然后点击页面右上角的「添加学生」按钮，填写学生基本信息（姓名、性别、班级、饭卡号等），最后点击「保存」即可完成添加。',
    },
    {
      question: '如何调整学生积分？',
      answer:
        '在学生列表中点击学生姓名进入详情页，或者在列表中找到对应学生后点击操作栏的「调整积分」按钮，输入调整的分数（正数为加分，负数为扣分）和备注说明，点击确认即可完成积分调整。',
    },
    {
      question: '积分规则如何设置？',
      answer:
        '进入「积分规则」页面，可以添加、编辑或删除积分规则。每个规则可以设置规则名称、所属分类、分数值、每日上限等参数。规则设置完成后，在给学生调整积分时可以选择对应的规则。',
    },
    {
      question: '如何使用MQTT调试工具？',
      answer:
        '进入「MQTT调试」页面，首先在「连接状态」标签页中点击「连接」按钮连接到MQTT服务器。连接成功后，可以在「消息发布」标签页发布消息，在「主题订阅」标签页订阅主题，在「消息日志」标签页查看消息记录，在「设备控制」标签页进行设备测试。',
    },
    {
      question: '手机箱如何验证学生积分？',
      answer:
        '当学生刷卡时，手机箱会通过MQTT发送查询消息到「phonebox/query」主题，后端系统收到查询后验证饭卡信息和积分，然后通过「phonebox/unlock/A」或「phonebox/unlock/B」主题返回授权结果。积分充足时允许开锁，积分不足时拒绝开锁。',
    },
    {
      question: '如何导入/导出积分规则？',
      answer:
        '在「积分规则」页面点击「导出规则」按钮可以将所有规则导出为JSON文件。点击「导入规则」按钮选择JSON文件可以批量导入规则。导入前请确保文件格式正确。',
    },
    {
      question: '排名等级是如何划分的？',
      answer:
        '排名等级可以在「排名规则」页面自定义设置。系统预置了四个等级：待达标(0-59分)、合格(60-79分)、优秀(80-89分)、卓越(90-100分)。您可以根据需要添加、修改或删除等级划分。',
    },
    {
      question: '数据分析页面展示哪些内容？',
      answer:
        '数据分析页面展示积分统计卡片（总学生数、平均分、最高分、最低分）、积分趋势图表、学生积分排行榜以及按班级统计的数据。帮助您全面了解积分分布情况。',
    },
  ];

  const filteredFaqs: Faq[] = faqs.filter(
    (faq: Faq) =>
          ...
          {docs.map((doc: Doc, index: number) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (doc.url) {
                    // P3-1: 文档卡打开真实链接；md 走新窗口，Swagger 同页打开
                    window.open(doc.url, doc.url.endsWith('.md') ? '_blank' : '_self');
                  }
                }}
