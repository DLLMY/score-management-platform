...
function UserList() {
  ...
  const getRankInfo = useCallback((score) => {
  ...
  const handleAddScore = useCallback(async (userId, score) => {
    try {
  ...
  const handleQuickScore = async (rule) => {
    try {
      ...
      dispatch({ type: 'UPDATE_USER_SCORE', payload: { userId: state.quickScoreUser.id, scoreChange: rule.score } });
  ...
  const handleBatchDelete = async () => {
    ...
    if (!window.confirm(`确定要删除选中的 ${state.selectedUsers.length} 名学生吗？此操作不可撤销。`)) {
      return;
    }
  ...
  const handleBatchScore = async () => {
    ...
    try {
      ...
      dispatch({ type: 'BATCH_UPDATE_SCORE', payload: { userIds: state.selectedUsers, scoreChange: state.batchScoreChange } });
  ...
  const renderUserRow = useCallback((user) => {
    ...
    return (
      <div className={`flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 sm:py-4 border-b border-gray-100 transition-all duration-300 ${
        ...
        <div className="flex-shrink-0 flex items-center gap-1">
          ...
          <Link to={`/users/${user.id}`} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-all duration-200 transform hover:scale-110">
            <Eye className="w-4 h-4" />
          </Link>
          <button
            onClick={() => { dispatch({ type: 'SET_EDITING_USER', payload: user }); dispatch({ type: 'SET_FORM_DATA', payload: user }); dispatch({ type: 'SET_SHOW_MODAL', payload: true }); }}
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5 mb-7">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button 
            onClick={() => { dispatch({ type: 'SET_EDITING_USER', payload: null }); dispatch({ type: 'SET_SHOW_MODAL', payload: true }); }}
          >
      ...
      {state.error && (
        <div className="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 flex items-center gap-3">
          ...
          <button 
            onClick={() => { dispatch({ type: 'SET_ERROR', payload: null }); fetchUsers(); }} 
      ...
      {state.pagination.pages > 1 && (
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
          <div className="text-sm text-gray-600">
            共 <span className="font-semibold">{state.pagination.total}</span> 条记录，当前第 <span className="font-semibold">{state.pagination.page}</span> / <span className="font-semibold">{state.pagination.pages}</span> 页
          </div>
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => dispatch({ type: 'SET_PAGINATION', payload: { page: state.pagination.page - 1 } })}
            >
              上一页
            </Button>
      ...
      <Modal 
        ...
        onClose={() => { dispatch({ type: 'SET_SHOW_MODAL', payload: false }); dispatch({ type: 'SET_EDITING_USER', payload: null }); }}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-50 border rounded-xl text-sm focus:outline-none focus:ring-2 ${
                  formErrors.name ? 'border-danger-300 focus:ring-danger-500' : 'border-gray-200 focus:ring-primary-500'
                }`}
                placeholder="请输入学生姓名"
              />
              {formErrors.name && (
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-50 border rounded-xl text-sm focus:outline-none focus:ring-2 ${
                  formErrors.class_name ? 'border-danger-300 focus:ring-danger-500' : 'border-gray-200 focus:ring-primary-500'
                }`}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-50 border rounded-xl text-sm focus:outline-none focus:ring-2 ${
                  formErrors.phone ? 'border-danger-300 focus:ring-danger-500' : 'border-gray-200 focus:ring-primary-500'
                }`}
                placeholder="请输入联系电话"
              />
              {formErrors.phone && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { father_name: e.target.value } })}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                  placeholder="请输入父亲姓名"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { father_phone: e.target.value } })}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                  placeholder="请输入父亲电话"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { mother_name: e.target.value } })}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                  placeholder="请输入母亲姓名"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { mother_phone: e.target.value } })}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                  placeholder="请输入母亲电话"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { guardian_name: e.target.value } })}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { guardian_phone: e.target.value } })}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                    placeholder="监护人电话"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => dispatch({ type: 'SET_FORM_DATA', payload: { guardian_relation: e.target.value } })}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                    placeholder="如：祖父"
                  />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-gray-50 border rounded-xl text-sm focus:outline-none focus:ring-2 ${
                  formErrors.card_id ? 'border-danger-300 focus:ring-danger-500' : 'border-gray-200 focus:ring-primary-500'
                }`}
                placeholder="请输入饭卡号"
              />
              {formErrors.card_id && (
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-100">
            <Button 
              variant="outline" 
              onClick={() => { dispatch({ type: 'SET_SHOW_MODAL', payload: false }); dispatch({ type: 'SET_EDITING_USER', payload: null }); }}
            >
              取消
            </Button>
            <Button type="submit">
              {state.editingUser ? '保存修改' : '添加学生'}
            </Button>
          </div>
        </form>
      </Modal>
        ...
        <div className="space-y-6">
          <div>
            ...
            <input
              ...
              onChange={(e) => dispatch({ type: 'SET_BATCH_SCORE_CHANGE', payload: parseInt(e.target.value) || 0 })}
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-100">
            <Button 
              variant="outline" 
              onClick={() => { dispatch({ type: 'SET_SHOW_BATCH_MODAL', payload: false }); dispatch({ type: 'SET_BATCH_SCORE_CHANGE', payload: 0 }); }}
            >
              取消
            </Button>
      ...
      >
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            ...
            <Button variant="outline" onClick={() => dispatch({ type: 'SET_SHOW_IMPORT_MODAL', payload: false })}>
              关闭
            </Button>
          </div>
        </div>
      ...
      <Modal
        ...
        onClose={() => { dispatch({ type: 'SET_SHOW_QUICK_SCORE_MODAL', payload: false }); dispatch({ type: 'SET_QUICK_SCORE_USER', payload: null }); }}
        ...
        <div className="space-y-6">
          {state.quickScoreUser && (
            <div className="flex items-center gap-4 p-4 bg-amber-50 rounded-xl border border-amber-200">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">{state.quickScoreUser.class_name} · 当前积分: <span className="font-bold text-amber-600">{state.quickScoreUser.current_score}分</span></p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {state.scoreTab === 'add' 
                ? state.rules.filter(r => r.is_active && r.score > 0).map(rule => (
                    <button
                      key={rule.id}
            ...
            {state.scoreTab === 'add' && state.rules.filter(r => r.is_active && r.score > 0).length === 0 && (
              <div className="text-center py-12 text-gray-500">
                <div className="text-sm">暂无可用的加分规则，请先创建规则</div>
              </div>
            )}
            {state.scoreTab === 'subtract' && state.rules.filter(r => r.is_active && r.score < 0).length === 0 && (
              <div className="text-center py-12 text-gray-500">
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-100">
            <Button 
              variant="outline" 
              onClick={() => { dispatch({ type: 'SET_SHOW_QUICK_SCORE_MODAL', payload: false }); dispatch({ type: 'SET_QUICK_SCORE_USER', payload: null }); }}
            >
              取消
            </Button>
          </div>
