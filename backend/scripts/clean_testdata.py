...
def ex(sql, *a): cur.execute(sql, a)

MARK = "%测试%","%CrudTest%","%E2E%","%API测试%","%全面测试%"

# 1) 测试学生 id
test_ids = [r["id"] for r in q("SELECT id FROM user WHERE role='student' AND (name LIKE ? OR name LIKE ? OR name LIKE ? OR name LIKE ? OR name LIKE ?)", *MARK)]
...
def collect_ids(table, cols):
    ids = set()
    for col in cols:
        for r in q(f"SELECT id FROM {table} WHERE {col} LIKE ? OR {col} LIKE ? OR {col} LIKE ? OR {col} LIKE ? OR {col} LIKE ?", *MARK):
            ids.add(r["id"])
    return sorted(ids)
...
if residue["study_group"]:
    ex("DELETE FROM study_group_member WHERE group_id IN (" + qm(len(residue["study_group"])) + ")", *residue["study_group"])
    ex("DELETE FROM study_group_score WHERE group_id IN (" + qm(len(residue["study_group"])) + ")", *residue["study_group"])
if residue["activity"]:
    ex("DELETE FROM activity_registration WHERE activity_id IN (" + qm(len(residue["activity"])) + ")", *residue["activity"])
if residue["culture_record"]:
    ex("DELETE FROM culture_item WHERE record_id IN (" + qm(len(residue["culture_record"])) + ")", *residue["culture_record"])
if residue["homework_assignment"]:
    ex("DELETE FROM homework_submission WHERE assignment_id IN (" + qm(len(residue["homework_assignment"])) + ")", *residue["homework_assignment"])
print("  已删父表关联子表行")
