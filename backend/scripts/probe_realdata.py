...
st, created = req(
    "POST", "/api/seating/charts", token=token, body={"name": "E2E真实互通验证", "class_info_id": 1}
)
...
if chart_id:
    st, seat = req(
        "POST",
        f"/api/seating/charts/{chart_id}/seats",
        token=token,
    ...
    print(
        ...
        sd.get("student_name"),
        "| class_id=",
        sd.get("class_id") or sd.get("class_info_id"),
    )
...
for label, path in [
    ("考勤", "/api/attendance/records?class_info_id=1"),
...
print("\n=== 结论 ===")
print(
    "若第2步 排座 student_id=1 且 student_name='陈洋' 且读回一致 -> 工作台真实写入/读取真实学生数据 ✅"
)
