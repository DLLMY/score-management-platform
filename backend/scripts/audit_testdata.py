...
if cid:
    ...
    print(
        ...
        st,
        "| 本chart class_name=",
        (mine[0].get("class_name") if mine else None),
        "| class_id=",
        (mine[0].get("class_id") if mine else None),
    )
    # 清理：删除刚建的空表（不留测试数据）
...
test_stu = [
    r["id"]
    for r in q(
        "SELECT id FROM user WHERE role='student' AND (name LIKE '%测试%' OR name LIKE '%CrudTest%' OR name LIKE '%E2E%' OR name LIKE '%API测试%')"
    )
]
...
tables = {
    ...
    "committee_term": [],
    "parent_contact": [],
    "homework_assignment": ["title"],
    "attendance": ["notes"],
    "study_group": ["name"],
    "mental_health_record": ["notes"],
    "alert": [],
    "activity": ["title"],
    "culture_record": ["title", "content"],
    "study_guide": ["title", "content"],
    "seating_seat": [],
    "duty_assignment": [],
    "homework_submission": ["notes"],
    "study_group_member": [],
    "activity_registration": [],
    "culture_item": ["content"],
    "phone_box_policy": [],
}
total = 0
for t, cols in tables.items():
    try:
        ...
        for col in cols:
            txt_hits += q(
                f"SELECT COUNT(*) c FROM {t} WHERE {col} LIKE '%测试%' OR {col} LIKE '%CrudTest%' OR {col} LIKE '%E2E%' OR {col} LIKE '%API测试%' OR {col} LIKE '%全面测试%'"
            )[0]["c"]
        # 关联测试学生
        stu_hits = 0
        scols = [
            r["name"]
            for r in q(f"PRAGMA table_info({t})")
            if r["name"] in ("student_id", "class_id", "class_info_id", "leader_id")
        ]
        for sc in scols:
            if test_stu:
