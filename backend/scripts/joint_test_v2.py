...
class IntegrationTest:
    ...
    def record(self, category, test_name, status, detail, elapsed_ms=0):
        self.results.append({
            "category": category, "test": test_name, "status": status,
    ...
    def test_page_switching(self):
        ...
        pages = [
            ("/dashboard", "数据概览", [("GET", "/dashboard/data", None)], ["total_users", "total_rules", "total_admins"]),
            ("/users", "学生管理", [("GET", "/users/", {"page":1,"per_page":10})], ["users", "total"]),
            ("/rules", "积分规则", [("GET", "/rules/", {"page":1,"per_page":10})], ["rules", "total"]),
            ("/rank-rules", "排名规则", [("GET", "/rank-rules/", None)], ["rules"]),
            ("/categories", "积分分类", [("GET", "/score-categories/", None)], ["categories"]),
            ("/nlp-management", "智能评分", [("GET", "/nlp/rules/", {"page":1,"per_page":10})], ["items", "total"]),
            ("/class-management", "班级管理", [("GET", "/classes/", None)], ["classes"]),
            ("/subject-management", "科目管理", [("GET", "/subjects/", None)], ["items", "subjects"]),
            ("/course-schedule", "课程表", [("GET", "/course-schedules/", None)], None),
            ("/class-period-settings", "课程节次", [("GET", "/class-periods/", None)], ["periods", "total"]),
            ...
            ("/permissions", "权限管理", [("GET", "/roles/", None), ("GET", "/rbac/permissions", None)], None),
        ]
        ...
        for page_path, page_name, apis, expected_keys in pages:
            ...
            if all_pass:
                ...
                self.record("页面切换", f"{page_name}({page_path})", "FAIL", "; ".join(details), total_time)
        ...
        for ep, label in endpoints:
            ...
            if len(responses) == 3:
                if len(set(responses)) == 1:
                    ...
                    pass_count += 1
                    self.record("状态保持", f"{label}({ep})", "PASS", "3次响应(含时间戳变化)", sum(times)/3)
            else:
    ...
    def test_button_interactions(self):
        ...
        # 使用正确的必填字段
        crud_tests = [
            ("积分规则", "/rules/", {"name": "测试规则_自动", "description": "测试", "score": 5, "category": "behavior", "type": "add"}, "id"),
            ("班委成员", "/committee/members", {"class_id": 1, "position": "测试委员", "student_id": 1, "responsibilities": "测试"}, "id"),
            ("家长联系", "/parent/contacts", {"father_name": "测试父亲", "mother_name": "测试母亲", "father_phone": "13800000001", "mother_phone": "13800000002", "address": "测试地址", "email": "test@test.com", "student_id": 1}, "id"),
            ("作业", "/homework/assignments", {"class_id": 1, "title": "测试作业", "due_date": "2026-08-15", "subject_id": 1, "description": "测试描述"}, "id"),
            ("文体活动", "/activity", {"class_id": 1, "title": "测试活动", "activity_type": "体育", "start_date": "2026-08-01"}, "id"),
            ("班级文化", "/culture/records", {"class_id": 1, "title": "测试文化", "content": "测试内容", "type": "class_spirit"}, "id"),
            ("学习小组", "/study-group/groups", {"class_id": 1, "name": "测试小组", "description": "测试"}, "id"),
            ("学法指导", "/study-guide/guides", {"class_id": 1, "title": "测试学法", "content": "测试内容", "subject": "数学"}, "id"),
        ]
    ...
    def _test_crud(self, label, endpoint, create_data, id_field):
        ...
        if err or (status and status >= 400):
            self.record("按钮-新增", f"{label}-新增", "FAIL", f"POST:{status} {str(err or body)[:40]}", elapsed)
            return
        ...
        if item_id:
            for op, method, suffix in [("详情", "GET", ""), ("修改", "PUT", ""), ("删除", "DELETE", "")]:
                url = f"{endpoint.rstrip('/')}/{item_id}{suffix}"
                if op == "修改":
                    update_data = dict(create_data)
                    if "name" in update_data: update_data["name"] += "_更新"
                    if "title" in update_data: update_data["title"] += "_更新"
                ...
                if op == "详情" and status in (404, 405):
                    ...
                    self.record(f"按钮-{op}", f"{label}-{op}", "WARN", f"无详情GET路由(可接受) {status}", elapsed)
                elif op == "删除" and status and status >= 400:
                    self.record(f"按钮-{op}", f"{label}-{op}", "FAIL", f"{status} {str(body)[:30]}", elapsed)
                elif status and status < 400:
    ...
    def _extract_id(self, body):
        if isinstance(body, dict):
            for k in ("id", "member_id", "contact_id", "group_id", "assignment_id", "activity_id", "record_id", "guide_id"):
                if k in body: return body[k]
            if "data" in body and isinstance(body["data"], dict):
                for k in ("id", "member_id", "contact_id", "group_id", "assignment_id", "activity_id", "record_id", "guide_id"):
                    if k in body["data"]: return body["data"][k]
        return None
    ...
    def test_data_coordination(self):
        ...
        test_cases = [
            ("班委成员", "/committee/members", {"class_id": 1, "position": "协同委员", "student_id": 1}),
            ("家长联系", "/parent/contacts", {"father_name": "协同父", "mother_name": "协同母", "father_phone": "13900000001", "mother_phone": "13900000002", "address": "协同地址", "email": "sync@test.com", "student_id": 1}),
        ...
        for label, ep, data in test_cases:
            ...
            if s2 and s2 < 400:
                ...
                if after > before:
                    self.record("数据协同", f"{label}-写入验证", "PASS", f"前:{before} 后:{after}", t1+t2+t3)
                else:
                    self.record("数据协同", f"{label}-写入验证", "FAIL", f"前:{before} 后:{after}", t1+t2+t3)
                if item_id:
                    ...
                    if final <= before:
                        ...
                        self.record("数据协同", f"{label}-删除验证", "WARN", f"预期:{before} 实际:{final}", 0)
            else:
    ...
    def _get_count(self, body):
        if isinstance(body, dict):
            ...
            for k in ("items", "members", "contacts", "groups", "rules", "devices"):
                if k in body and isinstance(body[k], list): return len(body[k])
            if "total" in body: return body["total"]
        elif isinstance(body, list): return len(body)
        return 0
    ...
    def test_performance(self):
        ...
        endpoints = [
            ("/dashboard/data", "仪表盘"), ("/users/", "学生列表"), ("/rules/", "积分规则"),
            ("/classes/", "班级列表"), ("/exams/", "考试列表"), ("/nlp/rules/statistics", "NLP统计"),
            ("/committee/members", "班委名单"), ("/seating/charts", "座次表"), ("/duty/groups", "值日生表"),
        ]
        for ep, label in endpoints:
            times = []
            for _ in range(3):
    ...
    def generate_report(self):
        ...
        for cat, counts in categories.items():
            ...
            print(f"{cat:<15} {counts['pass']:>6} {counts['fail']:>6} {counts['warn']:>6} {rate:>7.1f}%")
        print("-" * 50)
        print(f"{'总计':<15} {total_pass:>6} {total_fail:>6} {total_warn:>6} {total_pass/total*100 if total > 0 else 0:>7.1f}%")
        ...
        print("\n" + "=" * 90)
        report = {"test_time": datetime.now().isoformat(), "summary": {
            "total": total, "pass": total_pass, "fail": total_fail, "warn": total_warn,
            "pass_rate": f"{total_pass/total*100:.1f}%" if total > 0 else "0%"
        }, "categories": categories, "details": self.results}
