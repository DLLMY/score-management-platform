...
with app.app_context():
    ...
    tables_to_update = {
        'class_periods': {
            'columns': ['id', 'name', 'period_number', 'start_hour', 'start_minute', 
                       'end_hour', 'end_minute', 'description', 'is_active', 'sort_order',
                       'created_at', 'updated_at']
        },
        'notify_templates': {
            'columns': ['id', 'name', 'template', 'text', 'description', 'volume', 
                       'speak', 'popup', 'timeout_sec', 'urgent', 'bg_color', 'text_color',
                       'font_size', 'language', 'category', 'tags', 'usage_count', 
                       'is_active', 'created_by', 'created_at', 'updated_at']
        },
        'device_groups': {
            'columns': ['id', 'name', 'description', 'location', 'icon', 'color',
                       'sort_order', 'admin_id', 'is_active', 'created_at', 'updated_at']
        }
    }
        ...
        if table_name in inspector.get_table_names():
            ...
            if missing_columns:
                ...
                for col in missing_columns:
                    ...
                    if col in ['id', 'period_number', 'start_hour', 'start_minute', 'end_hour', 
                              'end_minute', 'sort_order', 'admin_id', 'created_by', 'usage_count',
                              'timeout_sec', 'font_size']:
                        col_type = 'INTEGER'
                    ...
                    try:
                        db.session.execute(db.text(f"ALTER TABLE {table_name} ADD COLUMN {col} {col_type}"))
