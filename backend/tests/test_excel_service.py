...
def sample_data():
    """生成测试数据 - 键名与表头一致"""
    return [
        {'姓名': '张三', '年龄': 20, '成绩': 85.5, '是否启用': True, '创建时间': datetime(2025, 1, 15, 10, 30)},
        {'姓名': '李四', '年龄': 21, '成绩': 92.0, '是否启用': False, '创建时间': datetime(2025, 2, 20, 14, 45)},
        {'姓名': '王五', '年龄': 22, '成绩': 78.3, '是否启用': True, '创建时间': datetime(2025, 3, 25, 9, 15)},
    ]
...
class TestExcelExportService:
    ...
    def test_export_to_excel_handles_none_values(self, sample_headers):
        ...
        result = ExcelExportService.export_to_excel(
            data=data, headers=sample_headers
        )
        assert isinstance(result, io.BytesIO)
    ...
    def test_export_to_csv(self, sample_data, sample_headers):
        result = ExcelExportService.export_to_csv(
            data=sample_data, headers=sample_headers
        )
    ...
    def test_export_large_dataset_with_progress(self):
        ...
        def data_getter(page, page_size):
            call_count[0] += 1
            return [{'id': i, 'name': f'学生{i}'} for i in range((page - 1) * page_size, page * page_size)]
    ...
    def test_export_to_excel_auto_column_width(self, sample_headers):
        data = [{'姓名': 'A' * 50, '年龄': 20, '成绩': 85, '是否启用': '是', '创建时间': '2025-01-01'}]
...
class TestExcelServiceEdgeCases:
    ...
    def test_large_batch_export_performance(self):
        ...
        def data_getter(page, page_size):
            ...
            return [{'id': i, 'value': f'item_{i}'} for i in range(start, start + page_size) if i < row_count]
