...
class TestDeviceWriteEndpoints:
    ...
    def test_create_device_missing_device_id_returns_error(self, client, app, auth_headers, db_session):
    ...
    def test_update_device(self, client, app, auth_headers, db_session):
        with app.app_context():
            ...
            resp = client.put(
                f'/api/devices/{d.id}', json={'name': '改名后'}, headers=auth_headers
            )
    ...
    def test_update_device_not_found(self, client, app, auth_headers, db_session):
        with app.app_context():
            resp = client.put(
                '/api/devices/99999', json={'name': 'x'}, headers=auth_headers
            )
