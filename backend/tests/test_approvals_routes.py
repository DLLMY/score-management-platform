...
class TestApprovalsRoutes:
    ...
    def test_pending_only_returns_pending(self, client, app, auth_headers, sample_user, db_session):
        with app.app_context():
            pending_a = Approval(
                student_id=sample_user.id, type="t", title="待审", score_change=1, status="pending"
            )
            approved_a = Approval(
                student_id=sample_user.id, type="t", title="已批", score_change=1, status="approved"
            )
    ...
    def test_approve_approval_success(self, client, app, auth_headers, sample_user, db_session):
        with app.app_context():
            ...
            notif = Notification.query.filter_by(
                student_id=sample_user.id, type="approval_result"
            ).first()
    ...
    def test_approve_approval_recalc_failure_isolated(
        self, client, app, auth_headers, sample_user, db_session
    ):
        """T1 回归：综合分重算异常被隔离，不得污染主数据提交。

        审批通过链路在 approve_approval 落库后触发 CompositeScoreService.recalculate_user_score；
        该步为最佳努力（失败不影响主流程）。此用例确保：即便重算抛异常，审批状态仍翻转、
        积分仍原子累加、ScoreRecord 仍落库——防止有人误将重算异常重抛导致审批半提交。
        """
        ...
        with app.app_context():
            ...
            assert body["success"] is True
            assert Approval.query.get(aid).status == "approved"
            assert User.query.get(sample_user.id).current_score == 60
            rec = ScoreRecord.query.filter_by(student_id=sample_user.id).first()
            assert rec is not None
            assert rec.score_change == 10
    ...
    def test_approve_no_score_change_does_not_mutate(
        self, client, app, auth_headers, sample_user, db_session
    ):
        with app.app_context():
            a = Approval(
            ...
            resp = client.post(
                "/api/approvals/%d/approve" % a.id, json={"approver_id": 1}, headers=auth_headers
            )
            assert resp.status_code == 200
            u = User.query.get(sample_user.id)
    ...
    def test_reject_approval_not_pending(self, client, app, auth_headers, sample_user, db_session):
        with app.app_context():
            ...
            db_session.commit()
            resp = client.post(
                "/api/approvals/%d/reject" % a.id, json={"approver_id": 1}, headers=auth_headers
            )
            assert resp.status_code == 400
    ...
    def test_batch_approve_success(self, client, app, auth_headers, sample_user, db_session):
        ...
        with app.app_context():
            assert Approval.query.get(ids[0]).status == "approved"
            assert Approval.query.get(ids[1]).status == "approved"
    ...
    def test_batch_approve_empty_ids(self, client, app, auth_headers):
        resp = client.post("/api/approvals/batch-approve", json={"ids": []}, headers=auth_headers)
        assert resp.status_code == 400
