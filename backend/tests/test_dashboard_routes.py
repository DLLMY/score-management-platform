...
class TestDashboardRoutes:
    ...
    def test_get_dashboard_data(self, client, session):
        ...
        device = Device(
            device_id="DEV001", name="测试设备", status="online", last_heartbeat=datetime.now()
        )
        session.add(device)
    ...
    def test_dashboard_category_stats(self, client, session):
        ...
        session.add(
            ScoreRecord(
                student_id=user.id, rule_id=rule1.id, score_change=5, description="规则1记录"
            )
        )
        session.add(
            ScoreRecord(
        ...
        session.add(
            ScoreRecord(
                student_id=user.id, rule_id=rule2.id, score_change=10, description="规则2记录"
            )
        )
        session.commit()
