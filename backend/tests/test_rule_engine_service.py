...
from services.rule_engine_service import RuleMatcher, ParameterMapper, RulePriorityEngine, RuleExecutionEngine
try:
    from app import app
except ImportError:
    pass
...
class TestRuleMatcher:
    ...
    def test_match_rules_above_threshold(self):
        ...
        with patch('services.rule_engine_service.get_by_id', return_value=mock_rule):
            model_output = {
                'recommendations': [
                    {'rule_id': 1, 'confidence': 0.998, 'conditions': {}}
                ]
            }
            result = matcher.match_rules(model_output, {})
            assert len(result) == 1
