...
def test_students(db_session):
    users = [User(name=f"班委生{i}-{id(object())}", card_id=f"CM{i}-{id(object())}") for i in range(4)]
...
class TestCommitteeService:
    def test_create_member(self, app, db_session, test_class, test_students):
        result = committee_service.create_member({
            "class_id": test_class.id, "position": "班长", "student_id": test_students[0].id,
    ...
    def test_list_members(self, app, db_session, test_class, test_students):
        committee_service.create_member({"class_id": test_class.id, "position": "学习委员", "student_id": test_students[1].id})
    ...
    def test_create_term(self, app, db_session, test_class):
        result = committee_service.create_term({
            "class_id": test_class.id, "term_name": "2026秋季",
            "start_date": date.today().isoformat(), "is_current": True
        })
        assert result[0]["success"] is True

    def test_list_terms(self, app, db_session, test_class):
        committee_service.create_term({"class_id": test_class.id, "term_name": "2026春季", "is_current": True})
