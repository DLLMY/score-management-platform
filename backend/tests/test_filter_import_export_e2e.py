...
class TestFilterFunctionality:
    ...
    def test_subject_filter_by_name(self, client, auth_headers, db_session):
        ...
        # Test: 按名称筛选
        response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"search": "数学"}
        )
    ...
    def test_subject_filter_by_status(self, client, auth_headers, db_session):
        """测试按状态筛选科目"""
        response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"include_inactive": "false"}
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_subject_filter_inactive(self, client, auth_headers, db_session):
        """测试获取包含已禁用的科目"""
        response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"include_inactive": "true"}
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_subject_combined_filters(self, client, auth_headers, db_session):
        ...
        response = client.get(
            ...
            query_string={"search": "高一", "include_inactive": "false"},
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_subject_filter_empty_result(self, client, auth_headers):
        """测试筛选无结果场景"""
        response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"search": "不存在的科目名称"}
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_subject_filter_chinese_characters(self, client, auth_headers, db_session):
        ...
        response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"search": "物理"}
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
...
class TestImportFunctionality:
    ...
    def test_import_subject_invalid_format(self, client, auth_headers):
        ...
        data = {"file": (io.BytesIO(invalid_content), "invalid.xlsx")}
        response = client.post(
            "/api/subjects/import",
            headers=auth_headers,
            data=data,
    ...
    def test_import_subject_invalid_extension(self, client, auth_headers):
        ...
        data = {"file": (io.BytesIO(invalid_content), "test.pdf")}
        response = client.post(
            "/api/subjects/import",
            headers=auth_headers,
            data=data,
    ...
    def test_import_subject_missing_required_field(self, client, auth_headers):
        ...
        data = {"file": (buf, "missing_field.xlsx")}
        response = client.post(
            "/api/subjects/import",
            headers=auth_headers,
            data=data,
        ...
        print(
            f'[PASS] 缺少必填字段被正确检测: 成功 {result.get("success_count", 0)}, 失败 {result.get("failed_count", 0)}'
        )
    ...
    def test_import_subject_duplicate_data(self, client, auth_headers, db_session):
        ...
        print(
            f'[PASS] 重复数据导入处理: 成功 {result.get("success_count", 0)}, 失败 {result.get("failed_count", 0)}'
        )

    def test_import_subject_valid_json(self, client, auth_headers):
        """测试有效的JSON格式导入"""
        import_data = [
            {"name": "JSON科目1", "code": "JSON001", "grade": "高一", "is_active": True},
            {"name": "JSON科目2", "code": "JSON002", "grade": "高二", "is_active": True},
        ]

        data = {"file": (io.BytesIO(json.dumps(import_data).encode()), "import.json")}
        response = client.post(
            "/api/subjects/import",
            headers=auth_headers,
            data=data,
    ...
    def test_import_exam_validation(self, client, auth_headers):
        """测试考试导入的格式验证"""
        data = {"file": (io.BytesIO(b"invalid"), "exam_import.xlsx")}
        response = client.post(
...
class TestExportFunctionality:
    ...
    def test_export_subject_excel(self, client, auth_headers, db_session):
        ...
        assert (
            ...
            or "excel" in response.content_type
    ...
    def test_export_subject_with_inactive(self, client, auth_headers, db_session):
        ...
        response = client.get(
            "/api/subjects/export",
            headers=auth_headers,
            query_string={"format": "excel", "include_inactive": "true"},
        )
        assert response.status_code == 200
    ...
    def test_export_exam_template(self, client, auth_headers):
        """测试模板下载"""
        response = client.get("/api/subjects/template", headers=auth_headers)
        assert response.status_code == 200
...
class TestImportExportCombined:
    ...
    def test_export_then_import_roundtrip(self, client, auth_headers, db_session):
        ...
        # Import (验证文件可以被正确解析)
        import_data = {"file": (io.BytesIO(export_response.data), "reimport.xlsx")}
        ...
        print(
            f'[PASS] 往返流程: 成功 {result.get("success_count", 0)}, 失败 {result.get("failed_count", 0)}'
        )
    ...
    def test_import_then_filter(self, client, auth_headers, db_session):
        ...
        data = {"file": (io.BytesIO(json.dumps(import_data).encode()), "filter_test.json")}
        import_response = client.post(
            "/api/subjects/import",
            headers=auth_headers,
            data=data,
        ...
        filter_response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"search": "筛选测试A"}
        )
    ...
    def test_filter_then_export(self, client, auth_headers, db_session):
        ...
        export_response = client.get(
            "/api/subjects/export",
            headers=auth_headers,
            query_string={"format": "excel", "search": "导出筛选A"},
        )
        assert export_response.status_code == 200
...
class TestErrorHandling:
    ...
    def test_filter_sql_injection_prevention(self, client, auth_headers):
        ...
        for payload in malicious_inputs:
            response = client.get(
                "/api/subjects/", headers=auth_headers, query_string={"search": payload}
            )
            assert response.status_code == 200
            data = response.get_json()
            # 不应该返回所有数据
    ...
    def test_import_oversized_file(self, client, auth_headers):
        ...
        data = {"file": (io.BytesIO(large_content), "large_file.xlsx")}
        response = client.post(
            "/api/subjects/import",
            headers=auth_headers,
            data=data,
    ...
    def test_import_without_auth(self, client):
        """测试未授权导入"""
        data = {"file": (io.BytesIO(b"test"), "test.xlsx")}
        response = client.post(
            "/api/subjects/import", data=data, content_type="multipart/form-data"
        )
        assert response.status_code in [401, 403]
    ...
    def test_filter_without_auth(self, client):
        """测试未授权筛选"""
        response = client.get("/api/subjects/", query_string={"search": "test"})
        assert response.status_code in [401, 403]
...
class TestPerformanceMetrics:
    ...
    def test_filter_response_time(self, client, auth_headers, db_session):
        ...
        subjects = [
            Subject(
                name=f"筛选性能测试{i}", code=f"FILTER_PERF{i:04d}", grade="高一", is_active=True
            )
            for i in range(50)
        ]
        db_session.add_all(subjects)
        ...
        start_time = time.time()
        response = client.get(
            "/api/subjects/", headers=auth_headers, query_string={"search": "筛选性能"}
        )
        elapsed_time = time.time() - start_time
