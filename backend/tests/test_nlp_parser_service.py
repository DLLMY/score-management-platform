...
class TestNLPParserService:
    ...
    def test_parse_add(self, app):
        with app.app_context():
            ...
            user = User(name="张三", card_id="NLP002", class_name="测试班级", current_score=100)
            keyword = NLPBehaviorKeyword(
                keyword="加分", keyword_type="奖励", score_type="add", default_score=5
            )
            rule = NLPScoringRule(
                behavior_keyword="加分",
    ...
    def test_parse_deduct(self, app):
        with app.app_context():
            ...
            user = User(name="李四", card_id="NLP003", class_name="测试班级", current_score=100)

            existing_keyword = NLPBehaviorKeyword.query.filter_by(keyword="迟到").first()
            if not existing_keyword:
                keyword = NLPBehaviorKeyword(
                    keyword="迟到", keyword_type="纪律", score_type="deduct", default_score=-5
                )
                db.session.add(keyword)
