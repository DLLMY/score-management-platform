...
def _reset_analyzer_singleton():
    """复位 NLPAlgorithmAnalyzer 单例指标，隔离用例之间的全局状态。

    根因：NLPAlgorithmAnalyzer 用 __new__ 实现**进程级单例**（cls._instance 缓存），
    本文件的断言形如「新建 analyzer → 记录 N 条 → 计数 == N」。若此前有其它测试文件
    （如 test_nlp_routes.py 的 train-all 异步流程）向同一单例写入过计数，这里就会
    读到 N+1（历史表现为 `assert 4 == 3`），构成顺序依赖失败。

    单例设计本身是合理的（全局指标聚合），因此不在生产代码里改动，
    只在测试侧于每个用例前后调用 reset_metrics() 复位，保证结果确定。
    """
    from services.nlp_analyzer_service import NLPAlgorithmAnalyzer
...
class TestNLPAlgorithmAnalyzer:
    ...
    def test_record_intent_prediction(self):
        ...
        analyzer.flush_request_metrics()
...
class TestAnalyzerRequestIsolation:
    """#990 / #929 单例污染修复：请求级缓冲 + 原子提交隔离回归。"""

    def test_record_buffers_before_flush(self):
        """record_* 应先写入线程局部缓冲，flush 前不污染全局聚合。"""
        analyzer = NLPAlgorithmAnalyzer()
        analyzer.reset_metrics()
        analyzer.record_intent_prediction("intent_a", "intent_a", 0.9)
        # flush 前全局未变更（请求级隔离）
        ...
        analyzer.flush_request_metrics()
        # flush 后原子提交到全局
        assert analyzer.intent_metrics.total_predictions == 1
        assert analyzer.intent_metrics.correct_predictions == 1
    ...
    def test_flush_idempotent(self):
        ...
        analyzer.flush_request_metrics()
        analyzer.flush_request_metrics()
        assert analyzer.performance_metrics.total_requests == 1

    def test_reset_clears_pending_buffer(self):
        """未 flush 的缓冲在 reset 后应被丢弃，不应泄漏到全局。"""
        analyzer = NLPAlgorithmAnalyzer()
        analyzer.reset_metrics()
        analyzer.record_intent_prediction("intent_x", "intent_x", 0.9)
        # 未提交即 reset
        analyzer.reset_metrics()
        assert analyzer.intent_metrics.total_predictions == 0
        # 后续 flush 不应再提交已被清空的缓冲
        analyzer.flush_request_metrics()
        assert analyzer.intent_metrics.total_predictions == 0

    def test_read_endpoints_flush_before_return(self):
        """get_* 读取端点应在返回前提交当前线程缓冲（契约不变）。"""
        analyzer = NLPAlgorithmAnalyzer()
        analyzer.reset_metrics()
        analyzer.record_intent_prediction("intent_a", "intent_a", 0.95)
        analyzer.record_performance(0.2)
        # 不显式 flush，直接读 → 端点内部 flush 应使数据可见
        report = analyzer.get_comprehensive_report()
        summary = report["intent_analysis"]["summary"]
