...
class TestAdminRoutes:
    ...
    def test_change_password_success(self, client, auth_headers, sample_admin, session):
        """测试修改密码成功"""
        response = client.post(f'/api/admins/{sample_admin.id}/change-password',
    ...
    def test_change_password_failure_wrong_old_password(self, client, auth_headers, sample_admin, session):
        """测试修改密码失败-旧密码错误"""
        response = client.post(f'/api/admins/{sample_admin.id}/change-password',
                               headers=auth_headers, json={
    ...
    def test_change_password_failure_weak_password(self, client, auth_headers, sample_admin, session):
        """测试修改密码失败-弱密码"""
        response = client.post(f'/api/admins/{sample_admin.id}/change-password',
                               headers=auth_headers, json={
            'old_password': 'testpassword123',
    ...
    def test_assign_class(self, client, auth_headers, sample_admin, sample_class, session):
        """测试分配班级"""
        response = client.post(f'/api/admins/{sample_admin.id}/assign-class',
                               headers=auth_headers, json={
