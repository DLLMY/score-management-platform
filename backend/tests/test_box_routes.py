...
class TestBoxRoutes:
    ...
    def test_box_verify_missing_params(self, client, app, auth_headers, db_session):
        with app.app_context():
            ...
            response = client.post(
                "/api/box/verify", json={"card_id": "123456"}, headers=auth_headers
            )
    ...
    def test_box_verify_no_rule(self, client, app, auth_headers, db_session):
        with app.app_context():
            ...
            db_session.add(device)
            user = User(
                name="无规则用户", card_id="BOXCARD_NR", class_name="二班", current_score=60
            )
            db_session.add(user)
            db_session.commit()
    ...
    def test_box_verify_rule_inactive(self, client, app, auth_headers, db_session, sample_rule):
        with app.app_context():
            ...
            db_session.add(device)
            user = User(
                name="未启用规则用户", card_id="BOXCARD_IA", class_name="四班", current_score=80
            )
            db_session.add(user)
            db_session.commit()
