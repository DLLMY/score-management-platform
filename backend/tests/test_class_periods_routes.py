class TestClassPeriodsRoutes:
    ...
    def test_create_class_period(self, client, app, auth_headers):
        with app.app_context():
            response = client.post(
                '/api/class-periods/',
                json={'name': '测试节次', 'period_number': 991, 'start_hour': 8,
                      'start_minute': 0, 'end_hour': 8, 'end_minute': 40},
    ...
    def test_create_duplicate_period_number(self, client, app, auth_headers):
        with app.app_context():
            r1 = client.post(
                '/api/class-periods/',
                json={'name': 'A节', 'period_number': 992, 'start_hour': 8,
                      'start_minute': 0, 'end_hour': 8, 'end_minute': 40},
                headers=auth_headers,
            )
            ...
            r2 = client.post(
                '/api/class-periods/',
                json={'name': 'B节', 'period_number': 992, 'start_hour': 9,
                      'start_minute': 0, 'end_hour': 9, 'end_minute': 40},
                headers=auth_headers,
            )
    ...
    def test_update_class_period(self, client, app, auth_headers):
        with app.app_context():
            cr = client.post(
                '/api/class-periods/',
                json={'name': '旧节次', 'period_number': 993, 'start_hour': 8,
                      'start_minute': 0, 'end_hour': 8, 'end_minute': 40},
                headers=auth_headers,
            )
            pid = cr.get_json()['data']['id']
            response = client.put(f'/api/class-periods/{pid}', json={'name': '新节次'}, headers=auth_headers)
            assert response.status_code == 200
    ...
    def test_delete_class_period(self, client, app, auth_headers):
        with app.app_context():
            cr = client.post(
                '/api/class-periods/',
                json={'name': '删节次', 'period_number': 994, 'start_hour': 8,
                      'start_minute': 0, 'end_hour': 8, 'end_minute': 40},
                headers=auth_headers,
            )
            pid = cr.get_json()['data']['id']
    ...
    def test_batch_update_class_periods(self, client, app, auth_headers):
        with app.app_context():
            cr = client.post(
                '/api/class-periods/',
                json={'name': '批节次', 'period_number': 995, 'start_hour': 8,
                      'start_minute': 0, 'end_hour': 8, 'end_minute': 40},
                headers=auth_headers,
            )
            pid = cr.get_json()['data']['id']
