...
class TestDataSyncService:
    ...
    def test_sync_class_name_change(self, app):
        with app.app_context():
            ...
            user1 = User(name="学生1", card_id="SYNC001", class_name="旧班级", current_score=100)
            user2 = User(name="学生2", card_id="SYNC002", class_name="旧班级", current_score=80)
            admin1 = Admin(
    ...
    def test_sync_class_name_change_same_name(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="班级A", grade="初一", is_active=True)
            db.session.add(class_info)
            db.session.commit()
    ...
    def test_sync_new_class_creation(self, app):
        with app.app_context():
            ...
            user1 = User(name="学生1", card_id="SYNC003", class_name="新建班级", current_score=100)
            admin1 = Admin(
            ...
            new_class = ClassInfo(name="新建班级", grade="初二", is_active=True)
            ...
            admin_class = AdminClass.query.filter_by(
                admin_id=updated_admin.id, class_info_id=new_class.id
            ).first()
    ...
    def test_sync_class_deletion(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="待删除班级", grade="初三", is_active=True)
            db.session.add(class_info)
            db.session.commit()
            ...
            admin_class = AdminClass(admin_id=1, class_info_id=class_info.id, is_primary=True)
    ...
    def test_sync_user_class_change(self, app):
        with app.app_context():
            ...
            user = User(name="学生4", card_id="SYNC005", class_name="原班级", current_score=100)
            db.session.add(user)
            db.session.commit()
    ...
    def test_sync_user_class_change_existing_class(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="已有班级", grade="初二", is_active=True)
            db.session.add(class_info)

            user = User(name="学生5", card_id="SYNC006", class_name="原班级", current_score=100)
            db.session.add(user)
            db.session.commit()
    ...
    def test_sync_user_class_change_remove(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="班级C", grade="初一", is_active=True)
            db.session.add(class_info)
    ...
    def test_sync_admin_class_change_existing(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="已有班级B", grade="初三", is_active=True)
            db.session.add(class_info)
    ...
    def test_get_class_students(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="班级E", grade="初一", is_active=True)
            db.session.add(class_info)
            db.session.commit()
    ...
    def test_get_class_teachers(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="班级F", grade="初二", is_active=True)
            db.session.add(class_info)
            db.session.commit()
            db.session.refresh(class_info)
    ...
    def test_get_teacher_classes(self, app):
        with app.app_context():
            ...
            class_info = ClassInfo(name="班级G", grade="初三", is_active=True)
            db.session.add(class_info)
            db.session.commit()
            db.session.refresh(class_info)
