...
def nlp_test_data(session):
    ...
    if not category_add:
        category_add = ScoreCategory(name='加分项', description='加分行为', color='#10B981', is_active=True)
        session.add(category_add)

    category_deduct = ScoreCategory.query.filter_by(name='扣分项').first()
    if not category_deduct:
        category_deduct = ScoreCategory(name='扣分项', description='扣分行为', color='#EF4444', is_active=True)
        session.add(category_deduct)

    session.commit()

    # 创建用户，避免重复
    existing_users = {u.card_id: u for u in User.query.filter(User.card_id.in_(['C001', 'C002', 'C003', 'C004',
        'C005'])).all()}
    users_to_create = []
    user_data = [
        ('张三', 'C001', 60),
    ...
    for name, card_id, score in user_data:
        if card_id in existing_users:
            ...
            user = User(name=name, card_id=card_id, class_name='测试班', current_score=score, is_active=True)
    ...
    # 创建行为关键词，避免重复
    existing_keywords = {kw.keyword: kw for kw in NLPBehaviorKeyword.query.filter(NLPBehaviorKeyword.keyword.in_([
        '迟到', '做好事', '作业没交', '帮助', '加分', '扣分', '扣', '加'])).all()}
    keywords_to_create = []
    keyword_data = [
        ('迟到', 'deduct'),
        ('做好事', 'add'),
    ...
    # 创建评分规则，避免重复
    existing_rules = {sr.behavior_keyword: sr for sr in NLPScoringRule.query.filter(NLPScoringRule.behavior_keyword.in_([  # noqa: E501
        '迟到', '做好事', '作业没交', '帮助同学'])).all()}
    rules_to_create = []
    rule_data = [
        ('迟到', 2, 'deduct'),
        ('做好事', 10, 'add'),
    ...
    for behavior_keyword, score_value, score_type in rule_data:
        if behavior_keyword in existing_rules:
            ...
            rule = NLPScoringRule(behavior_keyword=behavior_keyword, score_value=score_value, score_type=score_type)
...
class TestNLPPerformance:
    ...
    def test_nlp_warmup_improves_first_call(self, nlp_test_data):
        ...
        assert success_count >= len(test_texts) - 1, f"解析成功率不足，成功{success_count}/{len(test_texts)}"
    ...
    def test_nlp_parse_extracts_score(self, nlp_test_data):
        ...
        for text, expected_score in test_cases:
            ...
            if result.get('success'):
                ...
                assert abs(score) == abs(expected_score), f"解析分数错误: 期望{expected_score}, 实际{score}"
