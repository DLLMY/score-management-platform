...
class TestAnalysisService:
    ...
    def test_get_user_analysis_empty(self, app, sample_user, db_session):
        """测试获取用户分析 - 无数据（有效用户但无积分记录）"""
        from services.analysis_service import analysis_service

        with app.app_context():
            result = analysis_service.get_user_analysis(sample_user.id)
            assert isinstance(result, dict), "返回应为 dict"
            assert result["user_id"] == sample_user.id
            assert result["record_count"] == 0, "无数据用户记录数应为 0"
            assert result["total_add_score"] == 0
            assert result["total_subtract_score"] == 0
            assert result["recent_30_days_count"] == 0
