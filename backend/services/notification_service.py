...
class NotificationService:
    ...
    @staticmethod
    def send_wechat_notification(user_id: int, template_id: str, data: Dict,
                                  jump_url: Optional[str] = None) -> Dict:
        """
        发送微信模板消息
        ...
        """
        try:
            from models import db, User, Admin, OperationLog

            user = User.query.get(user_id)
            if not user or not user.parent_info:
                return {'success': False, 'message': '用户或家长信息不存在'}
...
def notify_score_change(user_id: int, change: int, reason: str) -> Dict:
    ...
    try:
        ...
        data = {
            'first': {'value': f'积分变动提醒', 'color': '#173177'},
