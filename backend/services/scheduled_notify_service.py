...
MQTT 下发循环保留在路由/任务层；事务边界（触发发送回滚守卫、后台任务单条失败回滚、
最终批量 commit）由本 service 自管（#629 全量收口，路由/任务零 db.session 触达）。
"""
...
def rollback_scheduled_session():
    """定时通知发送/处理失败的回滚守卫（#629 收口：路由/后台任务零 db.session 触达）。

    逐字节复刻原路由内联 db.session.rollback()（触发发送失败分支 + 后台任务单条失败分支）。
    """
    db.session.rollback()


def commit_scheduled_session():
    """后台任务 process_scheduled_notifications 最终统一提交（#629 收口）。
