...
ANOMALY_TYPES = {
    ...
    "group_anomaly": {
        "name": "群体异常",
        "threshold": 3,
        "description": "学生积分总量偏离班级均值超过3个标准差",
    },
    "frequency_anomaly": {
        "name": "频率异常",
        "threshold": 10,
...
class AnomalyDetector:
    ...
    def detect_batch(self, scores):
        ...
        for i, score in enumerate(scores):
            # AN3：留一法基线——剔除当前点后再算均值/标准差，
            # 避免单个大离群点抬高整体 std、压低自身 z 值导致漏检（基线自污染）。
            if n <= 2:
                ...
                if others_std == 0:
                    ...
                    z = abs((score - others_mean) / others_std)
                    is_anomaly = z > ANOMALY_TYPES["sudden_change"]["threshold"]
                    z_score = round(z, 3)
            if is_anomaly:
                anomalies.append(
