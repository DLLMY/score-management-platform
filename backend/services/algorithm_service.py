...
class AlgorithmService:
    ...
    def calculate_statistics(class_name=None):
        ...
        try:
            beh = df["behavior_score"].to_numpy()
            acad = df["academic_score"].to_numpy()
            n = len(beh)
            if n > 1:
                q1, q2 = np.percentile(beh, 33.33), np.percentile(beh, 66.67)
            else:
                q1 = q2 = beh[0]
            groups = {"低分组": [], "中分组": [], "高分组": []}
            for b, a in zip(beh, acad):
                if b <= q1:
                    groups["低分组"].append((b, a))
                elif b <= q2:
                    groups["中分组"].append((b, a))
                else:
                    groups["高分组"].append((b, a))
