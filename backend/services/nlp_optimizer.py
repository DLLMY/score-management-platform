...
class NLPPerformanceOptimizer:
    ...
    def warmup(self):
        ...
        try:
            ...
            # P0-3 修复：原引用未定义的 NLPMLService（被 except 静默吞错 → _nlp_ml_service 恒为 None）。
            # 改用真实类 NLPMLTrainingService；其 __init__ 已完成模型初始化，无 load_models 方法，
            # 故仅在存在该方法时调用（向后兼容）。
            ...
            logging.getLogger(__name__).error(
                f"[NLP优化器] ML模型预热失败，ML 服务将不可用: {e}", exc_info=True
            )
            self._nlp_ml_service = None
        try:
