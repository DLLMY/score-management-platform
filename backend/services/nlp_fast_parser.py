...
class FastNLPParser:
    ...
    # 分句分隔符。刻意不含「、」：枚举顿号是"共享谓语"结构
    # （如「张三、李四迟到扣2分」），拆开会让前半句失去谓语而解析失败。
    _CLAUSE_SPLIT_PATTERN = re.compile(r"[，,；;。！？!?]")

    def _split_clauses(self, text: str) -> list:
        """按分句分隔符切分文本，返回非空子句列表（保持原文顺序）。"""
    ...
    def parse(self, text: str) -> Dict[str, Any]:
        ...
        返回格式：
        {
            ...
            'name': str,          # 首个子句的姓名
            'score': float,       # 首个子句的分数
            'description': str,   # 首个子句的描述
            'confidence': float,  # 首个子句的置信度
            'parser_type': 'fast',
            'is_simple': bool,
            'is_compound': bool,  # 是否复句（子句数 > 1）
            'clause_count': int,  # 子句数量
            'results': list       # 逐子句结果，顺序与原文一致
        }

        复句处理：先按分句分隔符切分，再**逐子句独立解析**。

        此前是"整句统一抽一次意图/姓名/分数"，复句会出现
        「前半句的姓名 + 后半句的意图」的错配 —— 例如
        「张三迟到扣2分，李四积极回答加3分」被解析成「给**张三** *加* 2 分」，
        加减方向相反、李四被静默丢弃；而 confidence=1.0 让上层
        （nlp_service.py 的 confidence >= 0.6 门槛）直接采信，
        完整解析器根本不参与，全程无失败信号。

        拆分后：
        - 顶层字段取自第一个子句（语义自洽，不会张冠李戴）
        - results 给出全部子句，供支持多条记录的调用方使用
        """
        text = text.strip()
        if not text:
            return {
        ...
        results = [self._parse_single(clause) for clause in clauses]
        first = results[0]
        ...
        return {
            ...
            "intent": first["intent"],
            "name": first["name"],
            "extracted_name": first["extracted_name"],
            "score": first["score"],
            "description": first["description"],
            "confidence": first["confidence"],
            "parser_type": "fast",
            "is_simple": True,
            "is_compound": len(results) > 1,
