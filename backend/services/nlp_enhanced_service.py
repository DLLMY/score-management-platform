...
class EnhancedNLPParserService:
    ...
    def __init__(self):
        ...
        # BERT 语义相似度缓存（见 _compute_bert_similarities）
        # 规则向量按「规则文本签名」缓存（规则变更少，避免每次请求都过 BERT）；
        # 查询向量按输入文本缓存并限容，防止无限增长。
        self._bert_rule_embeddings = {"signature": None, "vectors": []}
        self._bert_text_cache = {}
        self._bert_text_cache_max_size = 512
    ...
    def _load_patterns_from_config(self):
        ...
        if config_patterns:
            ...
            self.name_patterns = [
                ...
                re.compile(r"^([\u4e00-\u9fa5]{2})(?:同学)?"),  # N2: 3字优先后的2字兜底
                re.compile(
        ...
        # #2 深化：内置同义词词典（含新增课堂口语/近义表达）与配置合并，
        # 配置优先、内置补位，确保增量词典在任意部署下都生效（避免被 `or` 短路丢弃）。
        _syn_default = {
            "迟到": ["晚到", "来晚", "迟到了", "迟到几分钟"],
            "早退": ["提前走", "提早走", "提前离开"],
            "旷课": ["没来", "缺席", "未到"],
            ...
            "协助": ["帮助", "帮忙", "支援"],
            # #2 深化：新增常见课堂口语/近义表达，提升同义词召回（纯增量）
            "主动参与": ["积极参加", "主动参加", "踊跃参与", "主动发言"],
            "积极参与": ["积极参加", "主动参加", "踊跃参与", "主动发言"],
            "主动学习": ["自觉学习", "自主学习", "积极学习"],
            "帮助他人": ["帮助同学", "协助他人", "乐于助人"],
            "友爱同学": ["团结同学", "友好同学", "关爱同学"],
            "尊师重道": ["尊敬老师", "尊重老师", "听老师话"],
            ...
            "有礼貌": ["懂礼貌", "讲文明", "尊敬他人"],
            "进步明显": ["进步很大", "提升明显", "进步快"],
            "上课专心": ["专心听讲", "认真听课", "注意力集中"],
            "作业工整": ["作业整洁", "书写认真", "卷面干净"],
            "关心集体": ["热爱集体", "集体荣誉感强", "为班争光"],
            "拾金不昧": ["归还失物", "不占便宜", "诚实"],
            "见义勇为": ["勇敢助人", "挺身而出", "正能量"],
            "勤俭节约": ["节约粮食", "不浪费", "珍惜资源"],
            "讲究卫生": ["爱干净", "勤洗手", "保持整洁"],
            "积极发言": ["主动回答", "踊跃发言", "举手发言"],
            "按时完成作业": ["准时交作业", "独立完成作业", "作业认真"],
            "诚实守信": ["诚实", "守信", "不说谎"],
            "乐于助人": ["热心助人", "主动帮忙", "关爱他人"],
            "专注学习": ["专心学习", "用心学习", "投入学习"],
            "乐观向上": ["积极乐观", "开朗阳光", "正能量"],
        }
        _syn_default.update(config_loader.get_synonym_expansion() or {})
        self.synonym_expansion = _syn_default
    ...
    def _refresh_cache(self):
        ...
        if now - self._cache["cache_time"] > self._cache["cache_ttl"]:
            ...
            self._cache["behavior_keywords"] = [
                {
                    "keyword": k.keyword or "",
                    "keyword_type": k.keyword_type or "",
                    "score_type": k.score_type or "",
                    "default_score": k.default_score,
                    "synonyms": k.synonyms or [],
                }
                for k in NLPBehaviorKeyword.query.all()
            ]
            self._cache["rules_by_intent"] = {}
            for intent in ["add", "deduct", "query", "reset"]:
                # 只缓存 rule.id（#912 实机：ORM 对象跨请求 detached 后访问属性
                # DetachedInstanceError；_get_rules_by_intent 每次 get_by_id 重新查询，
                # 消费点保持 ORM 语义零改动）
                self._cache["rules_by_intent"][intent] = [
                    r.id
                    for r in NLPScoringRule.query.filter(
    ...
    def _compute_rule_stats(self):
        ...
        for intent in ["add", "deduct", "query", "reset"]:
            # 缓存存 id 列表，这里走 _get_rules_by_intent 重新查询（cache_time 已更新不会递归）
    ...
    def _get_rules_by_intent(self, intent):
        self._refresh_cache()
        rule_ids = self._cache["rules_by_intent"].get(intent, [])
        # 缓存的是 id 列表，每次重新查询返回 session-bound 对象（避免跨请求
        # detached ORM 访问属性 500；主键查询轻量，规则量小可接受）
        rules = []
        for rid in rule_ids:
            rule = get_by_id(NLPScoringRule, rid)
            if rule is not None:
                rules.append(rule)
        return rules
    ...
    def _analyze_sentiment(self, text):
        # #2 深化：在原有词频加权基础上加入「否定翻转」与「程度副词加权」，
        # 使"不认真 / 没有迟到 / 非常优秀 / 很差"等语义被正确识别。
        # 仅做纯增量增强：不改变方法签名，返回区间仍为 [-1, 1]，零契约风险。
                ...
                entities["behaviors"].append(
                    {
                        ...
                        "start": text_no_name.index(_kw_text),
                        "end": text_no_name.index(_kw_text) + len(_kw_text),
                    }
                )
                    ...
                    if match:
                        ...
                        kw = next(
                            (
                                ...
                                if isinstance(k, dict) and k["keyword"] == pattern_name
                            ),
                            None,
                        )
                        if kw:
    ...
    def _get_bert_service_safe(self):
        ...
        延迟 import + 函数内取单例，便于测试注入替身（patch
        services.bert_service.get_bert_service 即可）。
        """
        try:
    ...
    def _bert_rule_vectors(self, service, rule_texts):
        """规则文本向量（按规则文本签名缓存，规则变更少，避免每次请求都过 BERT）。"""
        signature = "|".join(rule_texts)
        cache = self._bert_rule_embeddings
        if cache["signature"] == signature and len(cache["vectors"]) == len(rule_texts):
    ...
    def _bert_query_vector(self, service, text):
        """查询文本向量（按文本缓存并限容；超限整体清空，避免维护 LRU 的复杂度）。"""
        cached = self._bert_text_cache.get(text)
        if cached is not None or text in self._bert_text_cache:
            return cached
        try:
        ...
        self._bert_text_cache[text] = vector
        return vector

    def _compute_bert_similarities(self, text, rule_texts):
        """计算输入文本与各规则文本的 BERT 余弦相似度（0~1）。

        设计要点：
        - **降级安全**：BERT 不可用、向量缺失或形状不符 → 返回全 0，
          与接入前 `np.zeros` 的行为完全一致，不会引入新的失败模式。
        - **候选内相对归一**：余弦先取 `max(cos, 0)`，再除以候选集合中的最大值
          （与同一函数里 BM25 的 `s / max_bm25` 处理保持一致）。这样 BERT 表达的是
          "候选之间的相对语义贴近度"，避免中文句向量绝对余弦普遍偏高（0.5~0.7）
          导致几乎所有规则都被推过 0.15 门槛、丧失区分度。
        """
        count = len(rule_texts)
        if count == 0:
            return np.zeros(0)

        service = self._get_bert_service_safe()
        if service is None:
            return np.zeros(count)

        rule_vectors = self._bert_rule_vectors(service, rule_texts)
        query_vector = self._bert_query_vector(service, text)
        ...
        for vector in rule_vectors:
            if vector is None or getattr(vector, "shape", None) != query_vector.shape:
                similarities.append(0.0)
                continue
            rule_norm = float(np.linalg.norm(vector))
            if rule_norm <= 0:
                similarities.append(0.0)
                continue
            cosine = float(np.dot(vector, query_vector) / (rule_norm * query_norm))
            similarities.append(max(cosine, 0.0))
            has_valid = True
        ...
        return np.array([s / max_similarity for s in similarities], dtype=float)
    ...
    def _get_ml_service(self):
        ...
        仅在 ML 预测（ml_predict/ensemble_predict）被实际调用时才导入
        services.nlp_ml_service（其模块顶层 import torch），从而让 NLP 解析/
        评分热路径与启动预热均不加载 torch。
        """
        if self._ml_service is None:
            from services.nlp_ml_service import NLPMLTrainingService

            self._ml_service = NLPMLTrainingService()
        return self._ml_service

    def ml_predict(self, text):
        prediction = self._get_ml_service().predict(text)
        if prediction:
    ...
    def ensemble_predict(self, text):
        prediction = self._get_ml_service().ensemble_predict(text)
        if prediction:
            rule = get_by_id(NLPScoringRule, prediction.get("rule_id"))
            if rule and rule.is_active:
                return (
    ...
    def _split_compound_text(self, text):
        """按中英文标点拆分复合句为子句，过滤空串与纯空白。

        #991 复合句逐子句贯通评分：'张三加5分，李四扣3分' -> ['张三加5分', '李四扣3分']。
        仅按标点拆分（不含空格），避免破坏单意图短语中的空格。
        """
        import re
    ...
    def analyze_sentiment(self, text):
        ...
        # #2 深化：score / sentiment 标签改用带否定与程度副词的细粒度打分，
        # 使"不认真 / 没有迟到 / 非常优秀"等语义正确；返回契约（字段与区间）不变。
