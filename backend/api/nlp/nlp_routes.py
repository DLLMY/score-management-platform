...
class NLPModelTrain(Resource):
    ...
    def post(self):
        ...
        with _train_lock:
            ...
            if any(t["status"] == "running" for t in _train_tasks.values()):
            ...
            for tid in expired:
                _train_tasks.pop(tid, None)
            task_id = f"train-{next(_train_seq)}"
        ...
        def _run():
            try:
                ...
                if _train_tasks[task_id]["status"] != "error":
                    _train_tasks[task_id]["result"] = result
                    _train_tasks[task_id]["status"] = "done"
            except Exception as exc:  # 训练异常必须落状态，避免前端轮询永远 running
                logger.error("train 后台任务异常: %s", exc, exc_info=True)
                if _train_tasks[task_id]["status"] != "error":
                    _train_tasks[task_id]["error"] = str(exc)
                    _train_tasks[task_id]["status"] = "error"
            finally:
                _train_tasks[task_id]["finished_at"] = time.time()
...
class NLPModelTrainAll(Resource):
    ...
    def post(self):
        ...
        with _train_lock:
            ...
            for tid in expired:
                _train_tasks.pop(tid, None)
            task_id = f"trainall-{next(_train_seq)}"
            _train_tasks[task_id] = {
                "kind": "all",
                "status": "running",
                "result": None,
