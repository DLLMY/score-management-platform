...
# 学生流水最小字段子集（B3 扩展 2026-08-23，对应原 StudentRecords.get 内联 6 字段）
RECORD_MINIMAL_FIELDS = ["id", "score_change", "description", "operator", "rule_id", "created_at"]
...
def _serialize_student(user: User) -> dict:
    ...
    data = user.to_dict(STUDENT_LOGIN_FIELDS)
...
class StudentRecords(Resource):
    ...
    def get(self):
        ...
        data = [r.to_dict(RECORD_MINIMAL_FIELDS) for r in items]
...
class StudentInsights(Resource):
    ...
    def get(self):
        ...
        weeks = get_int_arg("weeks", default=8, min_val=1)
        uid = student.id
