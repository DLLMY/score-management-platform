...
class CategoryList(Resource):
    ...
    def get(self):
        ...
        rule_counts = count_by_fk(
            ScoreRule,
            ScoreRule.category_id,
            category_ids,
            ScoreRule.is_active.is_(True),
        )
        for cat in categories:
            result.append({**cat.to_dict(), "rule_count": rule_counts.get(cat.id, 0)})
...
class CategoryResource(Resource):
    ...
    def get(self, id):
        ...
        return {
            "success": True,
            "category": {**category.to_dict(), "rule_count": rule_count},
        }
