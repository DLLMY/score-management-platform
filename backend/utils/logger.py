...
def log_operation(operation_type, target_type=None, target_id=None, description=None, before_data=None, after_data=None, operator=None):
    """记录操作日志到数据库"""
    try:
        ...
        # 同时记录到日志文件
        log_info(f"操作日志: {operation_type} | {target_type} | {target_id} | {description}", 
                 operator=operator, ip=ip_address)
...
def log_access(endpoint=None, method=None, status_code=None, duration=None):
    ...
    try:
        ...
        log_info(f"API访问: {method} {endpoint} | 状态: {status_code} | 耗时: {duration}ms",
...
def log_security_event(event_type, description, **kwargs):
    ...
    try:
        ...
        # 记录日志
        security_logger.info(f"安全事件: {event_label} | {description}", extra={
...
def log_data_access(operation, data_type, record_count=0, admin_id=None):
    ...
    log_security_event(event_type, description, operation=operation, data_type=data_type, 
                       record_count=record_count, admin_id=admin_id)
...
def log_request_middleware(app):
    ...
    def before_request():
        ...
        log_debug(f"请求开始: {request.method} {request.path}",
    ...
    def after_request(response):
        if hasattr(request, 'start_time'):
            ...
            duration = None
        log_access(endpoint=request.path,
    ...
    def handle_exception(e):
        log_error(f"未处理异常: {e}", exception=e,
