...
time_rule_model = ns_time_rules.model('TimeRule', {
...
time_rule_response = ns_time_rules.model('TimeRuleResponse', {
...
time_rule_list_response = ns_time_rules.model('TimeRuleListResponse', {
    'rules': fields.List(fields.Nested(time_rule_response), description='时间规则列表')
})

time_rule_check_response = ns_time_rules.model('TimeRuleCheckResponse', {
    'allowed': fields.Boolean(description='是否允许操作'),
    'message': fields.String(description='消息'),
    'rule': fields.Nested(ns_time_rules.model('ActiveRule', {
        'id': fields.Integer(description='规则ID'),
        'name': fields.String(description='规则名称'),
        'allow_unlock': fields.Boolean(description='是否允许开锁')
    }))
...
class TimeRuleResource(Resource):
    ...
    def put(self, id):
        ...
        db.session.commit()
        return {'success': True, 'message': '时间规则更新成功'}
    ...
    def delete(self, id):
        ...
        db.session.commit()
        return {'success': True, 'message': '时间规则删除成功'}
