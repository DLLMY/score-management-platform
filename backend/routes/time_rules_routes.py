...
class TimeRuleList(Resource):
    @ns_time_rules.doc('list_time_rules')
    def get(self):
        ...
        return {
            'rules': [{
                ...
                'day_of_week_text': '每天' if r.day_of_week == -1 else ['周一', '周二', '周三', '周四', '周五', '周六', '周日'][r.day_of_week],
    ...
    def post(self):
        ...
        return {
            ...
            'day_of_week_text': '每天' if rule.day_of_week == -1 else ['周一', '周二', '周三', '周四', '周五', '周六', '周日'][rule.day_of_week],
...
class TimeRuleCheck(Resource):
    @ns_time_rules.doc('check_time_rule')
    def get(self):
        now = datetime.now()
