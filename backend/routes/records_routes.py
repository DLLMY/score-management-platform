...
from models import db, ScoreRecord, User
...
class RecordList(Resource):
    ...
    def post(self):
        ...
        log_operation(
            ...
            target_type="record",
            target_id=record.id,
            description=f'积分变动: {user_name} {"+" if data.get("score_change", 0) > 0 else ""}{data.get("score_change", 0)}分',  # noqa: E501
            after_data=data,
        )
...
class RecordByUser(Resource):
    ...
    def get(self, user_id):
        ...
        pagination = (
            ScoreRecord.query.filter_by(user_id=user_id)
