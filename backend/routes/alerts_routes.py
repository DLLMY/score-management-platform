...
alert_model = ns_alerts.model(
    "Alert",
    {
        ...
        "source": fields.String(description="告警来源"),
...
alert_stats_model = ns_alerts.model(
    "AlertStats",
    {
        ...
        "today": fields.Integer(description="今日告警数"),
    },
)  # noqa: E501
...
class AlertResource(Resource):
    ...
    def get(self, alert_id):
        ...
        if not alert:
            return ({"success": False, "message": "告警不存在"}, 404)
        return {
            "id": alert.id,
            "alert_type": alert.alert_type,
            "severity": alert.severity,
    ...
    def put(self, alert_id):
        ...
        db.session.commit()
        return {"success": True, "message": "告警状态更新成功"}
    ...
    def delete(self, alert_id):
        ...
        if not result:
            return ({"success": False, "message": "告警不存在"}, 404)
        return {"success": True, "message": "告警删除成功"}
...
class AlertReadAll(Resource):
    ...
    def post(self):
        ...
        return {"success": True, "message": f"已标记 {count} 条告警为已读", "count": count}
...
class AlertCleanup(Resource):
    ...
    def post(self):
        ...
        return {"success": True, "message": f"已删除 {count} 条过期告警", "deleted_count": count}
...
class AlertTest(Resource):
    ...
    def post(self):
        ...
        if alert:
            return {"success": True, "message": "测试告警创建成功", "alert_id": alert.id}
        else:
            return ({"success": False, "message": "测试告警创建失败"}, 500)
