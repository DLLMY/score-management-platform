...
from models import db, User, ClassInfo, cascade_delete_user_related_records
...
class UserList(Resource):
    ...
    def get(self):
        ...
        if search:
            query = query.filter(
                (User.name.like(f"%{search}%")) | (User.card_id.like(f"%{search}%")) | (User.phone.like(f"%{search}%"))
            )
        if class_name:
    ...
    def post(self):
        ...
        for user_id in ids:
            ...
            if user:
                ...
                record = ScoreRecord(
                    user_id=user_id, score_change=score_change, description=description, operator="batch_operation"
                )
                ...
                try:
                    ...
                    if existing:
                        ...
                        existing.guardian_relation = (
                            guardian_relation if guardian_relation else existing.guardian_relation
                        )
