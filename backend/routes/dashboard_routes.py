...
class DashboardData(Resource):
    ...
    def get(self):
        ...
        weekly_records = ScoreRecord.query.filter(ScoreRecord.created_at >= last_7_days).count()
        ...
        category_stats = (
            db.session.query(ScoreRecord.rule_id, db.func.sum(ScoreRecord.score_change))
        ...
        result = {
            ...
            "top_users": [
                {"id": u.id, "name": u.name, "class_name": u.class_name, "current_score": u.current_score}
                for u in top_users
            ],
