...
def check_rule_limit(user_id, rule_id):
    ...
    if rule.min_interval > 0:
        last_record = (
            ScoreRecord.query.filter(ScoreRecord.user_id == user_id, ScoreRecord.rule_id == rule_id)
...
def handle_mqtt_message(client, topic, message):
    ...
    # 添加到内存日志（用于实时显示）
    mqtt_logs.append(
        {"topic": topic, "message": message, "direction": "receive", "timestamp": datetime.now().isoformat()}
    )
        ...
        try:
            ...
            with app.app_context():
                ...
                if not user:
                    ...
                    record = ScoreRecord(
                        user_id=user.id, score_change=-10, description="开锁扣积分", operator="MQTT系统"
                    )
                ...
                if not user:
                    ...
                    if not rule or not rule.is_active:
                        ...
                        if not limit_check["allow"]:
                            ...
                            before_score = user.current_score  # noqa: F841
                            ...
                            if msg_id:
                                processed = ProcessedMessage(
                                    message_id=msg_id, record_id=record.id, new_score=new_score, client_id=client_id
                                )
                    ...
                    rule = ScoreRule.query.filter(
                        ScoreRule.name.like(f"%{rule_name}%"), ScoreRule.is_active == True  # noqa: E712
                    ).first()
                    if not rule:
                        ...
                        if matching_rules:
                            ...
                            response = {
                                "success": False,
                                "message": f'未找到启用的规则 "{rule_name}"，可能需要启用: {rule_names}',
                                "msg_id": msg_id,
                            }
                        else:
                            response = {
                                "success": False,
                                "message": f'未找到包含 "{rule_name}" 的规则',
                                "msg_id": msg_id,
                            }
                        publish_mqtt(response_topic, json.dumps(response))
                    else:
                    ...
                    _before_score = user.current_score  # noqa: F841
                    ...
                    response = {
                        "success": False,
                        "message": "需要提供 rule_id、rule_name 或 score_change",
                        "msg_id": msg_id,
                    }
                    publish_mqtt(response_topic, json.dumps(response))
        except Exception as e:
...
mqtt_unlock_model = ns_mqtt.model("MQTTUnlock", {"box_id": fields.String(description="箱子ID（A/B）")})
...
class MQTTConnect(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            print(
                f"使用配置: broker={config_dict['broker']}, port={config_dict['port']}, transport={config_dict['transport']}"  # noqa: E501
            )
