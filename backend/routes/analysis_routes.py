...
class UserAnalysis(Resource):
    ...
    def get(self, user_id):
        ...
        recent_records = ScoreRecord.query.filter(
            ScoreRecord.user_id == user_id, ScoreRecord.created_at >= last_30_days
        ).all()
...
class ClassAnalysis(Resource):
    ...
    def get(self, class_name):
        ...
        weekly_records = (
            ...
            .filter(User.class_name == class_name, ScoreRecord.created_at >= last_7_days)
            .all()
        )
...
class UnlockStats(Resource):
    ...
    def get(self):
        ...
        _device_id = request.args.get("device_id")  # noqa: F841
        class_name = request.args.get("class_name")

        query = ScoreRecord.query.filter(ScoreRecord.description.like("%开锁%"))
...
class ClassRanking(Resource):
    ...
    def get(self):
        ...
        return {"ranking": ranking_data[:limit], "total_classes": len(ranking_data)}
...
class DashboardSummary(Resource):
    ...
    def get(self):
        ...
        total_students = User.query.filter(User.is_active == True, User.class_name.isnot(None)).count()  # noqa: E712

        _total_devices = (  # noqa: F841
            db.session.query(func.count(db.func.distinct(db.column("device_id")))).scalar() or 0
        )
        ...
        class_list = (
            ...
            .filter(User.class_name.isnot(None), User.is_active == True)  # noqa: E712
            .group_by(User.class_name)
            .all()
        )
        ...
        return {
            ...
            "week": {"unlock_count": week_unlock, "avg_daily": round(week_unlock / max(1, today.weekday() + 1), 1)},
            "month": {
                "unlock_count": month_unlock,
                "total_cost": month_cost,
                "avg_daily": round(month_unlock / max(1, today.day), 1),
            },
            "totals": {"users": total_users, "students": total_students, "classes": len(class_list)},
