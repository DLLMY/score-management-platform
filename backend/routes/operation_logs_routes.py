...
class OperationLogStats(Resource):
    ...
    def get(self):
        ...
        unlock_a_count = query.filter(
            OperationLog.description.ilike('%开A箱%')
        ).count()

        unlock_b_count = query.filter(
            OperationLog.description.ilike('%开B箱%')
        ).count()
        ...
        by_day = db.session.query(
            ...
            func.sum(
                db.case(
                    (OperationLog.description.ilike('%成功%'), 1),
                    else_=0
                )
            ).label('success'),
            func.sum(
                db.case(
                    (OperationLog.description.ilike('%失败%'), 1),
                    else_=0
                )
            ).label('failure')
        ).filter(
            OperationLog.created_at >= datetime.fromisoformat(start_time) if start_time else True,
            OperationLog.created_at <= datetime.fromisoformat(end_time) if end_time else True
        ).group_by(func.date(OperationLog.created_at)).order_by(func.date(OperationLog.created_at).desc()).limit(30).all()
