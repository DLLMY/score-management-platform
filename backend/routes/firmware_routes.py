...
firmware_version_model = ns_firmware.model('FirmwareVersion', {
...
class OTACheck(Resource):
    ...
    def get(self):
        ...
        latest_firmware = FirmwareVersion.query.filter(
...
class OTAReport(Resource):
    ...
    def post(self):
        ...
        if status == 'started':
            ...
            update_record = DeviceFirmwareUpdate.query.filter_by(
                device_id=device_id,
                to_version=to_version,
                status='in_progress'
...
class BatchUpgrade(Resource):
    ...
    @ns_firmware.expect(ns_firmware.model('BatchUpgradeRequest', {
    ...
    def post(self):
        ...
        for device_id in device_ids:
            mqtt_manager.publish_ota_command(device_id, {
                'version': firmware.version,
        ...
        return {
            'success': True,
            'message': f'已向 {len(device_ids)} 台设备下发升级指令',
            'results': results
        }
...
class FirmwareUpload(Resource):
    ...
    @ns_firmware.expect(ns_firmware.model('FirmwareUpload', {
        'version': fields.String(required=True, description='固件版本号'),
        'description': fields.String(description='版本描述'),
        'min_compatible_version': fields.String(description='最低兼容版本'),
