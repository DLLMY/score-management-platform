...
from models import db, Exam, Score, User, ClassInfo, cascade_delete_related_records
...
exam_model = ns_exam.model(
    "Exam",
    {
        ...
        "subjects": fields.List(fields.String, description="考试科目"),
        "start_time": fields.DateTime(description="开始时间"),
        "end_time": fields.DateTime(description="结束时间"),
        "importance": fields.Integer(description="重要程度"),
...
class ExamDetail(Resource):
    ...
    def delete(self, id):
        ...
        try:
            ...
            # scores.exam_id 为 NOT NULL 外键，先清理关联成绩
            cascade_delete_related_records(Exam, id)
    ...
    def post(self):
        ...
        try:
            ...
            existing_score = Score.query.filter_by(
                exam_id=data["exam_id"], student_id=data["student_id"], subject=data["subject"]
            ).first()
            if existing_score:
            ...
            for score_data in scores_data:
                try:
                    ...
                    score = Score(
                        ...
                        status="pending",
                        entered_by=score_data.get("entered_by"),
                    )
                    db.session.add(score)
                    success_count += 1
                except Exception as e:
...
class ScoreConfirmAll(Resource):
    ...
    def post(self, exam_id):
        """确认所有成绩"""
        try:
            Score.query.filter_by(exam_id=exam_id).update({"status": "confirmed", "updated_at": datetime.now()})
            db.session.commit()
...
class ExamAnalysis(Resource):
    ...
    def get(self, exam_id):
        ...
        try:
            ...
            result = {"exam_id": exam_id, "exam_name": exam.name if exam else "", "subject_stats": {}, "overall": {}}
            all_scores = []
            for subject, score_list in subject_stats.items():
                if not score_list:
                    continue
