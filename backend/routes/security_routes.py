...
from models import db, Admin, OperationLog
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not signature or not timestamp:
                return {'success': False, 'message': '缺少签名信息'}, 401

            try:
                timestamp_int = int(timestamp)
                if abs(datetime.now().timestamp() - timestamp_int) > 300:
                    log_security_event('invalid_signature', 'warning', details='签名已过期')
                    return {'success': False, 'message': '签名已过期'}, 401
            except ValueError:
                return {'success': False, 'message': '无效的时间戳'}, 401
            ...
            if not hmac.compare_digest(signature, expected_signature):
                ...
                return {'success': False, 'message': '签名验证失败'}, 401
...
security_audit_model = ns_security.model('SecurityAudit', {
...
class SecurityAuditStats(Resource):
    ...
    def get(self):
        ...
        severity_counts = db.session.query(
            ...
            db.func.count(SecurityAudit.id)
        ).group_by(SecurityAudit.severity).all()
        ...
        type_counts = db.session.query(
            ...
            db.func.count(SecurityAudit.id)
        ).filter(SecurityAudit.created_at >= last_7d).group_by(SecurityAudit.event_type).all()
        ...
        top_ips = db.session.query(
            ...
            db.func.count(SecurityAudit.id).label('count')
        ).filter(
            SecurityAudit.created_at >= last_24h,
            SecurityAudit.severity.in_(['warning', 'error', 'critical'])
        ).group_by(SecurityAudit.ip_address).order_by(db.text('count DESC')).limit(10).all()
...
class SuspiciousIPs(Resource):
    ...
    def get(self):
        ...
        suspicious_ips = db.session.query(
            ...
            SecurityAudit.created_at >= last_1h
        ).group_by(SecurityAudit.ip_address).having(
            db.func.count(SecurityAudit.id) > threshold
        ).all()
...
class ClearRateLimit(Resource):
    ...
    def delete(self, ip=None):
        ...
        if not ip:
            return {'success': False, 'message': '请提供IP地址'}, 400
...
class VerifyToken(Resource):
    ...
    def post(self):
        ...
        if not token:
            return {'success': False, 'message': '请提供令牌'}, 400
