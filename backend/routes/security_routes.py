...
def rate_limit(max_requests=100, window_minutes=1):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            record = (
                RateLimitRecord.query.filter_by(ip_address=ip, endpoint=endpoint)
                .filter(RateLimitRecord.window_start >= window_start)
                .first()
            )
...
class SecurityAuditStats(Resource):
    ...
    def get(self):
        ...
        type_counts = (
            ...
            .group_by(SecurityAudit.event_type)
            .all()
        )
...
class ClearRateLimit(Resource):
    ...
    def delete(self, ip=None):
        ...
        return {"success": True, "message": f"已清除 {deleted} 条记录"}
...
class VerifyToken(Resource):
    ...
    def post(self):
        ...
        if is_valid:
            ...
            return {"valid": False, "message": result}, 401
