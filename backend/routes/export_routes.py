...
from models import User, ScoreRule, Device, ScoreRecord
...
export_format_model = ns_export.model(
    "ExportFormat",
    {
        "format": fields.String(required=True, description="导出格式：excel 或 pd"),
    ...
    def post(self):
        ...
        try:
            if export_type == "users":
                ...
                _ = ScoreRecord.query.order_by(ScoreRecord.created_at.desc()).limit(10000).all()
                record_data = [
                    {
                        "id": r.id,
                        "user_id": r.user_id,
...
class ExportRecords(Resource):
    ...
    @ns_export.doc(
        ...
        description="导出积分记录",
        security="Bearer",
        params={"format": "导出格式：excel（默认）或 pdf", "limit": "限制导出记录数量（默认10000）"},
    )
    @ns_export.response(200, "导出成功")
    @requires_admin
    def get(self):
        ...
        if export_format not in ["excel", "pdf"]:
            return ({"success": False, "message": "不支持的导出格式"}, 400)
        _ = ScoreRecord.query.order_by(ScoreRecord.created_at.desc()).limit(limit).all()
        record_data = [
            {
                "id": r.id,
                "user_id": r.user_id,
