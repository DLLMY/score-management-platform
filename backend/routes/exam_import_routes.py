...
from models import db, Exam, Score, User, Admin
    ...
    def post(self):
        ...
        try:
            ...
            if not validation["valid"]:
                return jsonify({"success": False, "message": "文件格式验证失败", "errors": validation["errors"]}), 400
...
class PreviewImportData(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            validation = ScoreImportHelper.validate_excel_format(sheet)
            if not validation["valid"]:
                return jsonify({"success": False, "message": "文件格式错误", "errors": validation["errors"]}), 400
        ...
        try:
            ...
            for i, row in enumerate(sheet.iter_rows(min_row=2, values_only=True)):
                ...
                try:
                    ...
                    score_val = ScoreImportHelper.parse_score_value(row[score_idx]) if score_idx >= 0 else None
                    full_score = (
                        ScoreImportHelper.parse_score_value(row[full_score_idx]) if full_score_idx >= 0 else 100
                    )
                    remark = str(row[remark_idx]).strip() if remark_idx >= 0 and row[remark_idx] else None
