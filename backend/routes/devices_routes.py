...
class DeviceAlerts(Resource):
    ...
    def get(self):
        ...
        if resolved:
            query = query.filter(DeviceAlert.is_resolved == True)
        else:
            query = query.filter(DeviceAlert.is_resolved == False)
        ...
        return {
            'alerts': [{
                ...
                'device_name': Device.query.filter_by(device_id=a.device_id).first().name if Device.query.filter_by(device_id=a.device_id).first() else None,
...
class DeviceAlertHistory(Resource):
    ...
    def get(self, id):
        ...
        alerts = DeviceAlert.query.filter_by(
            device_id=device.device_id
        ).order_by(DeviceAlert.created_at.desc()).limit(50).all()
        ...
        return {
            ...
            'total': len(alerts),
            'unresolved_count': DeviceAlert.query.filter_by(
                device_id=device.device_id, is_resolved=False
            ).count()
        }
...
class DeviceRemoteControl(Resource):
    ...
    @ns_devices.expect(ns_devices.model('RemoteControl', {
    ...
    def post(self, id):
        ...
        if action == 'restart':
            ...
            if result:
                return {'success': True, 'message': 'A箱开锁指令已发送', 'action': action, 'device_id': device.device_id}
            else:
                return {'success': False, 'message': 'MQTT发送失败，请检查连接'}, 500
            ...
            if result:
                return {'success': True, 'message': 'B箱开锁指令已发送（手动开锁）', 'action': action, 'device_id': device.device_id}
            else:
                return {'success': False, 'message': 'MQTT发送失败，请检查连接'}, 500
...
class DeviceAdvancedStats(Resource):
    ...
    def get(self):
        ...
        avg_signal = db.session.query(func.avg(Device.wifi_signal)).filter(
            Device.wifi_signal.isnot(None)
        ).scalar() or 0

        alert_count = DeviceAlert.query.filter_by(is_resolved=False).count()
        critical_alerts = DeviceAlert.query.filter_by(
        ...
        devices_with_signal = Device.query.filter(
            Device.wifi_signal.isnot(None)
        ).all()
...
class DeviceSettings(Resource):
    ...
    @ns_devices.expect(ns_devices.model('DeviceSettings', {
...
class BatchDeviceControl(Resource):
    ...
    @ns_devices.expect(ns_devices.model('BatchControl', {
...
ota_upgrade_model = ns_devices.model('OTAUpgrade', {
