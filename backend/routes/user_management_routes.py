...
from services.unlock_validator import (
    ...
    set_daily_unlock_limit, check_user_blacklist,
    UnlockValidator
)

ns_user_management = Namespace('user-management', description='用户管理增强功能')

blacklist_request = ns_user_management.model('BlacklistRequest', {
    'reason': fields.String(description='禁用原因'),
    'until': fields.DateTime(description='禁用截止时间（可选）')
})

unlock_limit_request = ns_user_management.model('UnlockLimitRequest', {
    'limit': fields.Integer(required=True, description='每日开锁次数限制')
})

user_blacklist_response = ns_user_management.model('UserBlacklistResponse', {
...
user_unlock_status_response = ns_user_management.model('UserUnlockStatusResponse', {
...
class UserUnlockLimit(Resource):
    @ns_user_management.doc('get_unlock_limit', description='获取用户开锁限制')
    @ns_user_management.response(200, '成功')
