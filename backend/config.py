...
【配置体系职责边界（P2 收敛说明）】
- 本文件 config.py 的 `Config` 类 = **Web 应用配置的权威源**（Flask/DB/Redis/MQTT/密钥等），
  由 `app/config_init.py::init_config` 灌入 `app.config`，业务代码统一读 `current_app.config`。
- `config/__init__.py` = 兼容层：`from config import config` 再导出常用键（向后兼容，勿再新增导出）。
- `config/config_loader.py` = **NLP/算法专属参数源**（intent_words / position_weights /
  keyword_importance / CACHE_TTL 等，数据来自 config/nlp_algorithm.py 与 nlp_keywords.json），
  仅供 algorithm_service / nlp_enhanced_service 使用，不参与 Web 密钥体系。
- 密钥（FLASK_SECRET_KEY / JWT_SECRET_KEY）只从环境变量读取；`utils/security.py` 的
  `JWT_SECRET_KEY` 为模块级 env 快照（生产缺失即 raise，fail-closed）；`app/config_init.py`
  `validate_secret_keys` 在启动时统一强校验（生产缺失拒绝启动）。三处读取点值同源、
  均 fail-closed，职责如上，改动任一处须同步其余两处。
"""
