...
def validate_secret_keys(app):
    ...
    FLASK_SECRET_KEY 与 JWT_SECRET_KEY 均要求显式通过环境变量提供：
    不允许依赖 config.py 的 _generate_secret_key() 自动生成临时密钥启动，
...
def _check_rbac_consistency(app):
    ...
    try:
        ...
        if code == 0:
            logger.info("[启动检查] RBAC 一致性 OK")
            return
        logger.warning("⚠️  [启动检查] RBAC 一致性存在问题（非致命，服务继续启动）：")
        for i in _issues:
            logger.warning("   - %s", i)
...
def _check_redis_connectivity(app):
    ...
    try:
        ...
        r.ping()
        logger.info("[R1] Redis 连通性 OK (%s)", url)
...
def init_config(app, lightweight=False):
    ...
    app.csrf = csrf
    logger.info("CSRF保护已 %s", "启用" if app.config.get("WTF_CSRF_ENABLED") else "禁用")
    ...
    if not config_ok and _current_flask_env(app) == "production":
        ...
        logger.critical("🚨 生产环境配置校验失败，拒绝启动！详见上方配置验证报告。")
        sys.exit(1)
    elif not config_ok:
        logger.warning("⚠️  配置校验存在错误（非生产环境仅警告，服务继续启动）")
