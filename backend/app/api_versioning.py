...
def _register_api_error_handlers(api):
    """给 Flask-RESTX Api 注册统一错误信封。

    Flask-RESTX 默认把 404/405/400（expect 校验失败）等输出为裸 {"message": 英文技术文案}，
    无 success 键。这里统一输出 {success:false, message: 友好中文, error_code}，与 app 级
    error_handler（utils/error_handler.py）及前端 getErrorMessage 的 error_code 文案表对齐。
    注：Flask-RESTX 的 Api.errorhandler 只接受异常类（不接受状态码 int），HTTPException
    统一走 code 分支。
    """
    ...
    def _handle_http_error(e):
        code = e.code or 500
        _messages = {
            400: ("请求参数错误，请检查输入内容", "BAD_REQUEST"),
            401: ("未授权，请登录", "UNAUTHORIZED"),
            403: ("您没有权限执行此操作", "FORBIDDEN"),
            404: ("请求的资源不存在", "NOT_FOUND"),
            405: ("请求方法不被允许", "METHOD_NOT_ALLOWED"),
            429: ("请求过于频繁，请稍后再试", "RATE_LIMITED"),
            500: ("服务器内部错误", "INTERNAL_ERROR"),
        }
        message, error_code = _messages.get(code, ("请求失败", f"HTTP_{code}"))
        return {"success": False, "message": message, "error_code": error_code}, code
...
class APIVersionManager:
    ...
    def init_app(self, app, register_default=True):
        # 关闭 Flask-RESTX 的 404 帮助文案注入（默认 True 会把 message 拼上
        # "You have requested this URI ... did you mean ..." 技术尾巴）
        ...
        for version, routes_func in self.versions.items():
            ...
            if register_default and version == self._default_version:
                ...
                _register_api_error_handlers(api_default)
...
def register_v1_routes(api, app):
    ...
    from api.class_management.teacher_comment_routes import ns_teacher_comment
    ...
    api.add_namespace(ns_teacher_comment, path="/teacher-comments")
